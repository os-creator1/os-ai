    ...
    @if(isset($menu))
        @foreach($menu as $submenu)
            @php
                ...
                // FIX: Explode permission string for @canany check
                $permission = explode('|', $submenu->access);
            @endphp

            {{-- FIX: Use @canany to handle pipe-separated permissions --}}
            @canany($permission, auth()->user())
                <li class="{{ $custom_classes }} {{ (isset($submenu->submenu)) ? 'dropdown dropdown-submenu' : '' }} {{ isset($submenu->slug) && str_contains(request()->path(),$submenu->slug) ? 'active' : '' }}" @if(isset($submenu->submenu))
