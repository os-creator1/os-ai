...
@section('content')
    <div class="row mb-2">
        <div class="col-12 d-flex justify-content-between align-items-center">
            ...
            <x-button variant="outline" size="sm" :href="route('customer.outreach.index')" icon="edit-3">
        ...
        @else
            <x-table :headers="[__('locale.labels.name'), 'Channel', __('locale.labels.status'), __('locale.labels.date')]">
                @foreach($campaigns as $campaign)
                    <tr>
                        <td>
                            <a href="{{ route('customer.outreach.campaigns.show', $campaign->uid) }}">{{ $campaign->campaign_name }}</a>
                        </td>
                        <td>
