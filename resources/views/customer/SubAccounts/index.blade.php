...
@section('title', __('locale.labels.sub_accounts'))
...
@section('content')
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            @if(config('account.delete_subaccount'))
            ...
            @if(config('account.create_subaccount'))
                <div class="btn-group">
                    <a href="{{route('customer.sub_accounts.create')}}"
    ...
    <script>
      $(document).ready(function() {
        ...
        let dataListView = $(".datatables-basic").DataTable({
          ...
          "serverSide": true,
          "ajax": {
            "url": "{{ route('customer.sub_accounts.search') }}",
            "dataType": "json",
            "type": "POST",
          ...
          columnDefs: [
            ...
            },
            {
              ...
              render: function(data, type, full) {
                ...
                return (
                  "<span class=\"action-delete text-danger pe-1 cursor-pointer\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=" + full["delete_label"] + " data-id=" + full["delete"] + ">" +
        ...
        Table.delegate(".get_status", "click", function() {
          let subAccount = $(this).data("id");
          $.ajax({
            url: "{{ url('/sub-accounts') }}/" + subAccount + "/active",
            type: "POST",
            data: {
              _token: "{{ csrf_token() }}"
            },
        ...
        Table.delegate(".action-delete", "click", function(e) {
          ...
          Swal.fire({
            ...
            if (result.value) {
              $.ajax({
                url: "{{ url('/sub-accounts')}}" + "/" + id + "/destroy",
                type: "POST",
                data: {
                  _token: "{{csrf_token()}}"
                },
        ...
        $(".bulk-enable").on("click", function(e) {
          ...
          Swal.fire({
            title: "{{__('locale.labels.are_you_sure')}}",
            text: "{{__('locale.sub_accounts.enable_selected_sub_accounts')}}",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.value) {
              ...
              $.each(rows_selected, function(index, rowId) {
                subAccountsId.push(rowId);
              });

              if (subAccountsId.length > 0) {

                $.ajax({
                  url: "{{ route('customer.sub_accounts.batch_action') }}",
                  type: "POST",
                  data: {
                    _token: "{{csrf_token()}}",
                    action: "enable",
        ...
        $(".bulk-disable").on("click", function(e) {
          ...
          Swal.fire({
            title: "{{__('locale.labels.are_you_sure')}}",
            text: "{{__('locale.sub_accounts.disable_selected_sub_accounts')}}",
            icon: "warning",
            showCancelButton: true,
        ...
        $(".bulk-delete").on("click", function(e) {
          ...
          Swal.fire({
            title: "{{__('locale.labels.are_you_sure')}}",
            text: "{{__('locale.sub_accounts.delete_selected_sub_accounts')}}",
            icon: "warning",
            showCancelButton: true,
