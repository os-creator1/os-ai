...
@section('content')
    ...
    @if(count($accessible) === 0)
        <x-card :padded="true">
            <x-empty-state icon="globe" title="No Business available yet"
                            description="Website Generation is organized by Business. You don't have access to a Business yet — ask an {{ $accountNoun }} owner to add you, or create a Business to get started." />
        </x-card>
    @else
        <x-card :padded="true">
            ...
            <div class="list-group">
                @foreach($accessible as [$workspace, $business])
                    <a href="{{ route('customer.workspaces.businesses.website.show', [$workspace->uid, $business->uid]) }}"
