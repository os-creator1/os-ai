...
@section('content')
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            <div class="btn-group">
                <a href="{{route('customer.automations.create')}}" class="btn btn-success waves-light waves-effect fw-bold"> {{__('locale.buttons.add_new')}} <i data-feather="plus-circle"></i></a>
            </div>
                ...
                <div class="card">
                    <table class="table datatables-basic">
                        ...
                        <tr>
                            ...
                            <th style="width: auto">{{__('locale.labels.status')}}</th>
    ...
    <script>
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('customer.automations.search') }}",
                    "dataType": "json",
                    "type": "POST",
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full) {
                            ...
                            if (full['automation_status'] === 'active') {
                                $actions += '<span class="action-disable text-danger cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="top" title=' + full['disable'] + ' data-id=' + full['uid'] + '>' +
                                    feather.icons['pause-circle'].toSvg({class: 'font-medium-4'}) +
                                    '</span>';
                            } else {
                                $actions += '<span class="action-enable text-success cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="top" title=' + full['enable'] + ' data-id=' + full['uid'] + '>' +
                                    feather.icons['play-circle'].toSvg({class: 'font-medium-4'}) +
                                    '</span>';
                            }
            ...
            // On Enable
            Table.delegate(".action-enable", "click", function (e) {
                e.stopPropagation();
                let id = $(this).data('id');
                Swal.fire({
                    ...
                    confirmButtonText: "{{ __('locale.labels.enable') }}",
                    customClass: {
            ...
            // On Disable
            Table.delegate(".action-disable", "click", function (e) {
                e.stopPropagation();
                let id = $(this).data('id');
                Swal.fire({
                    title: "{{ __('locale.labels.are_you_sure') }}",
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('/automations/')}}" + '/' + id + '/disable',
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
            ...
            $(".bulk-enable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.automations.bulk_enable_automations')}}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        ...
                        $.each(rows_selected, function (index, rowId) {
                            automations_id.push(rowId)
                        });

                        if (automations_id.length > 0) {

                            $.ajax({
                                url: "{{ route('customer.automations.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'enable',
            ...
            $(".bulk-disable").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.automations.bulk_disable_automations')}}",
                    icon: 'warning',
                    showCancelButton: true,
            ...
            $(".bulk-delete").on('click', function (e) {
                ...
                Swal.fire({
                    title: "{{__('locale.labels.are_you_sure')}}",
                    text: "{{__('locale.automations.bulk_delete_automations')}}",
                    icon: 'warning',
                    showCancelButton: true,
