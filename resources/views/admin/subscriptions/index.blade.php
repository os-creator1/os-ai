    ...
    <div class="row match-height">
        ...
        <div class="col-lg-4 col-sm-6 col-12">
            <div class="card">
                <div class="card-header">
                    <div>
                        <h2 class="fw-bolder mb-0">{{ isset($subscriptions_stats->total) ? $subscriptions_stats->total: '0' }}</h2>
                        <p class="card-text">{{ __('locale.labels.total') }}</p>
                    </div>

                    <div>
                        <i class="font-large-3 text-primary" data-feather="list"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-4 col-sm-6 col-12">
            <div class="card">
                <div class="card-header">
                    <div>
                        <h2 class="fw-bolder mb-0">{{ isset($subscriptions_stats->active) ? $subscriptions_stats->active: '0' }}</h2>
                        <p class="card-text">{{ __('locale.labels.active') }}</p>
                    </div>

                    <div>
                        <i class="font-large-3 text-success" data-feather="check-square"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-4 col-sm-6 col-12">
            <div class="card">
                <div class="card-header">
                    <div>
                        <h2 class="fw-bolder mb-0">{{ isset($subscriptions_stats->inactive) ? $subscriptions_stats->inactive: '0' }}</h2>
                        <p class="card-text">{{ __('locale.labels.inactive') }}</p>
                    </div>
                    <div>
                        <i class="font-large-3 text-danger" data-feather="x-square"></i>
                    </div>
    ...
    <script>
        $(document).ready(function () {
                ...
                columnDefs: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full) {
                            ...
                            if (full['is_new'] === true) {
                                $return_url +=   '<span class="action-approve text-warning cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="top" title=' + full['approve_label'] + ' data-id=' + full['uid'] + '>' +
                                    feather.icons['check-square'].toSvg({class: 'font-medium-4'}) +
                                    '</span>'+
                                    '<span class="action-cancel text-warning cursor-pointer px-1" data-bs-toggle="tooltip" data-bs-placement="top" title=' + full['cancel_label'] + ' data-id=' + full['uid'] + '>' +
            ...
            Table.delegate(".action-approve", 'click', function (e) {
                ...
                Swal.fire({
                    title: "{{ __('locale.labels.are_you_sure') }}",
                    text: "{{ __('locale.subscription.approve_subscriptions') }}",
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/subscriptions')}}" + '/' + id + '/approve-pending',
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
