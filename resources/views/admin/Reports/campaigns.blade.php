...
@section('content')
    ...
    <section id="datatables-basic">
        <div class="mb-3 mt-2">
            ...
            <div class="btn-group">
                <a href="{{route('admin.reports.campaign.export')}}"
                ...
                <div class="card">
                    <table class="table datatables-basic">
                        ...
                        <tr>
                            ...
                            <th style="width: 10%;">{{__('locale.labels.actions')}}</th>
                        </tr>
                        </thead>
                    </table>
                </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            let dataListView = $('.datatables-basic').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ route('admin.reports.search.campaigns') }}",
                    "dataType": "json",
                    "type": "POST",
                    ...
                    },
                    {
                        ...
                        render: function (data, type, full) {
                            ...
                            if (full['camp_status'] !== 'queued' && full['camp_status'] !== 'queuing' && full['camp_status'] !== 'scheduled') {
                                $actions += '<a href="' + full['overview'] + '" class="text-primary cursor-pointer me-1" data-bs-toggle="tooltip" data-bs-placement="top" title=' + full['overview_label'] + '>' +
                                    feather.icons['bar-chart'].toSvg({class: 'font-medium-4'}) +
                                    '</a>';
                            }

                            if (full['camp_status'] === 'sending' || full['camp_status'] === 'paused' || full['camp_status'] === 'error') {
                                $actions += '<span class="action-delivered text-success cursor-pointer me-1" data-bs-toggle="tooltip" data-bs-placement="top" title="' + full['mark_delivered'] + '" data-uid=' + full['uid'] + '>' +
            ...
            Table.delegate(".action-delete", "click", function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/reports/campaigns')}}" + '/' + id + '/delete',
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
            ...
            // On Delivered
            Table.delegate(".action-delivered", "click", function (e) {
                e.stopPropagation();
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url(config('app.admin_path').'/reports/campaigns')}}" + '/' + id + '/mark-delivered',
                            type: "POST",
                            data: {
                                _token: "{{csrf_token()}}"
                            },
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        if (campaigns_id.length > 0) {
                            ...
                            $.ajax({
                                url: "{{ route('admin.reports.campaign.batch_action') }}",
                                type: "POST",
                                data: {
                                    _token: "{{csrf_token()}}",
                                    action: 'destroy',
