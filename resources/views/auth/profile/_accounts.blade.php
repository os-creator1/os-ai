...
@if( ! Auth::user()->is_super_admin && config('account.can_delete'))
    <div class="card">
            ...
            <form id="formAccountDeactivation" class="validate-form" onsubmit="return false">
                <div class="form-check">
                    <input
                            ...
                            data-msg="{{ __('locale.auth.want_to_delete_your_account') }}"
                    />
                    <label class="form-check-label font-small-3" for="accountActivation">
                        {{ __('locale.auth.confirm_my_account_deactivation') }}
                    </label>
                </div>
                <div>
                    <button type="submit" class="btn btn-danger deactivate-account mt-1">{{ __('locale.auth.deactivate_account') }}</button>
                </div>
            </form>
        </div>
    </div>
