...
Grounded in the actual repository implementation of RFC-005 Milestones 1–5, Amendment 1 (Usage Meter Identity), and all ten post-M5 remediations, verified by direct inspection during Milestone 6. Not copied from any prior RFC's deployment guide.

**Corrected during M6:** this guide's own first draft predated RFC-005 Remediation 10 (Webhook Payload Retention Correction) and, separately, was found on independent re-audit to contain several citation/attribution errors of its own (a nonexistent method name, a misattributed remediation, an imprecise signature-verification claim, a stale line-number citation). Both are corrected throughout this document per `docs/automation/RFC-005-M6-DOCUMENTATION-CORRECTION-CONTRACT.md`'s own exhaustive audit — see §10 below for the corrected webhook-retry/reclaim/purge section in particular.
...
- **Ten post-M5 remediations** — reservation admission wiring, funding provider-flow correction, receipt boundary, job/event dispatch completion, reconciliation-race correction, funding-confirmation concurrency correction, admin usage-billing surface, provider refund/dispute-outcome handling, §35 test-coverage completion, and webhook payload retention correction (Remediation 10, discovered during this M6 pass's own first draft, fixing a real defect where unset retention purged rather than retained webhook payloads — §10 below).
...
- `USAGE_BILLING_WEBHOOK_RETENTION_DAYS` has **no default**. Leaving it unset means the purge job (`PurgeExpiredWebhookPayloads`) never purges any encrypted webhook payload — this fails closed (indefinite retention), never the opposite (immediate purge). Set an explicit value before relying on payload purging for storage/compliance reasons. **This guarantee is enforced by `PurgeExpiredWebhookPayloads::resolvedRetentionDays(): ?int`** (mirroring `Kernel::opportunitySnoozeSweepCronMinutes()`'s own validation idiom): it accepts only a positive integer or a digit-only string representing one; unset, blank, zero, negative, or malformed configuration all resolve to `null`, and `handle()` returns before any purge-candidate query executes whenever this is `null`. **This is the result of RFC-005 Remediation 10 (Webhook Payload Retention Correction)** — before that fix, an unset value cast via `(int)` to `0`, and `purgeable(0)` computed a cutoff of "now," causing nearly every already-terminal webhook payload to be purged on the very next hourly run, the exact opposite of the fail-closed behavior this bullet describes. That defect was found during this M6 pass's own first draft, separately contracted, authorized, implemented, and merged before this guide was corrected.
...
Confirmed directly in `app/Console/Kernel.php` (lines 26–32 for the `use` imports, lines 112–127 for the `$schedule->job(...)` registrations), inside the same `else` branch (line 77) that runs whenever the application is not in `demo` stage — i.e. these run in every real deployment, including local/staging/production, without a separate opt-in.
...
- Signature verification is performed exactly once, inside `StripeWebhookController@handle`, before any row is persisted — delegated to `PaymentProviderGateway::verifyWebhookSignature()` (implemented by `StripePaymentProviderGateway`), using `STRIPE_WEBHOOK_SECRET` and `STRIPE_WEBHOOK_TOLERANCE` (default 300 seconds) from `config/services.php`. A verification failure returns HTTP 400 with zero side effects — no row is ever inserted. **`ProcessPaymentProviderEvent` performs no signature verification of any kind** — it operates entirely on the already-persisted, already-verified row the controller created.
...
- **Claim/lease**: `ProcessPaymentProviderEvent` claims a `payment_provider_events` row under a lease (`USAGE_BILLING_WEBHOOK_LEASE_MINUTES`, default 5 minutes). A stale (expired) lease is eligible for reclaim by a fair, round-robin reclaim query (`EloquentPaymentProviderEventRepository::retryable()`) rather than a naive oldest-first scan that could starve later events. **This fair round-robin design belongs to RFC-005 Remediation #6 — Provider Refund/Dispute Outcome Handling (mechanically R8 in the ten-remediation sequence), confirmed by both the implementing code's own comments and `git log --follow` on the file — not to the Reconciliation-Race Correction remediation (R5), which fixes an entirely unrelated stale-in-memory-object race in `ReconcileProviderPendingState`/`UsageBillingCheckoutManager::confirmSucceeded()`.**
- **Retry/exhaustion**: `RetryStuckPaymentProviderEvents` (scheduled every 5 minutes) reclaims stuck/failed events, bounded by `PaymentProviderEventRetryPolicy::normalizedMaxAttempts()`, which clamps the configured `USAGE_BILLING_WEBHOOK_MAX_ATTEMPTS` to a hard ceiling of `MAX_ATTEMPTS_CEILING = 20` regardless of configuration. Once an event's attempt count is exhausted, it becomes permanently unreclaimed by the ordinary retry path and surfaces in the administrator's exhausted-events queue.
...
- **Purge**: `PurgeExpiredWebhookPayloads` (scheduled hourly) purges the encrypted payload (`payload_encrypted`) of `processed`/`ignored`/`disposed` events once past `USAGE_BILLING_WEBHOOK_RETENTION_DAYS` — but never their `id`/`provider_event_id`/`payload_hash`/`state`/`attempts`/timestamps/disposition fields, which remain permanently intact so the `UNIQUE (provider, provider_event_id)` idempotency constraint continues to reject a genuine replay even after payload purge. **This job never purges anything if `USAGE_BILLING_WEBHOOK_RETENTION_DAYS` is left unset** (§3 above) — enforced by `resolvedRetentionDays(): ?int`, the RFC-005 Remediation 10 correction.
- **Reconciliation**: `ReconcileProviderPendingState` (every 5 minutes) reconciles locally-pending funding attempts/slot agreements against the provider's own authoritative state, for cases where a webhook was missed or delayed rather than explicitly failed — this is the job the Reconciliation-Race Correction remediation (R5) fixed a stale-in-memory-object race in.
