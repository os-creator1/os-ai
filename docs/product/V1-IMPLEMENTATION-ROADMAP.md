...
- **Objective:** add the two missing RFC-005 §16 consent rules and activate `PayerType::AgencyRebill` under them (Addendum §10): only the managing Agency Workspace owner may set/revoke it or configure funding, gated on an active Slice 1 relationship, using the existing standing-consent mechanism. **Correction (contract remediation pass):** this slice also introduces a single canonical `EffectivePayerResolver` consumed by `BillingProfileManager`, `UsageBillingCheckoutManager`, `UsageWalletManager`, and `PaymentInstrumentManager` — a full trace found unhandled/silently-mishandled `PayerType` branches in all four classes, including one site (`UsageBillingCheckoutManager::initiateCharge()`) that would silently charge the client's own instrument instead of the Agency's. **Implementation-time correction:** the re-inventory at `00377d7f` found further payer branches (a second provider-customer resolver in `PaymentInstrumentManager`, a third consent copy in `UsageWalletManager`, `BillingProfileManager`'s payer-controls/presentation facts, the Workspace aggregate controls, the presenter and two controller labels) and no spend-time account-access check; `BillingProfileManager` becomes the single human payer/funding authority seam alongside `EffectivePayerResolver`. See Implementation Contract 09's implementation-time correction, §3 and §5 for the complete fourteen-site inventory and design.
- **Why here:** structurally requires Slice 1 (relationship) **and Slice 5** (Agency/Client effective-access composition) — **Slice 5 is now a hard prerequisite, not merely a benefit**, since `EffectivePayerResolver`'s `AgencyRebill` branch and every automated Agency-funded effect both depend on Slice 5's composed effective-access to gate spend correctly.
- **Sections:** Blueprint §20, §28; Addendum §10.
- **Code domains:** `app/Enums/Usage/PayerType.php` (gains the exhaustive `isGovernedByOwnWorkspaceControls()`/`countsTowardOwnWorkspaceAggregateLimits()` spend-scope methods), `app/Library/Usage/EffectivePayer.php` (extended) + new `EffectivePayerResolver`, `BillingProfileManager` (single human payer/funding authority seam), `UsageBillingCheckoutManager` (charge resolution + spend-time gate), `UsageWalletManager` (`isWorkspacePaid()` and all its consumers, auto-recharge consent copy, spend-time gate), `PaymentInstrumentManager` (provider-customer resolution; consent copy deleted), `UsageBillingPresenter` and the `WorkspaceController`/`UsageBillingController` payer labels, RFC-005 §16 consent table (doc update alongside code). Contract 09 §12 is the exact allowlist.
...
- **Billing impact:** Critical — this is real money movement; every payer-independent wallet/cap/entitlement/STOP-DND/provider-readiness/idempotency check applies unchanged (Addendum §10), while the payer-sensitive ones change per Contract 09 §5/§11: AgencyRebill paid effects additionally require standing consent and usable effective account access, and the Client Workspace's aggregate controls do not apply to AgencyRebill.
- **Prerequisites:** Slice 1 (hard); **Slice 5 (hard — upgraded from "recommended" in the contract remediation pass)**.
- **Blocks:** Slice 10 (a hard prerequisite — Slice 10's payer migration matrix requires `AgencyRebill` to already be a legal target, and Slice 10's cutover also depends on this slice's own new `agency_rebill_consented_at`/`_by_user_id` columns, added here per Contract 09 §5).
...
| `payer_type = 'workspace'`, under an Agency-tier Workspace, for a **non-primary** Business (i.e. a client) | The Agency's shared Workspace funding was paying for this client's usage — functionally identical to what `AgencyRebill` now exists to represent | **Corrected in the contract remediation pass — no longer a whole-Business halt (the original design had a chicken-and-egg bug: it required the Slice 1 relationship to already exist for re-consent, but the relationship wasn't created until after this step).** The Business's split still completes fully — Workspace, Business move, and Slice 1 relationship are all created immediately — but the row is converted directly to `payer_type = 'agency_rebill'` (referencing the now-existing relationship) with its new `agency_rebill_consented_at`/`_by_user_id` columns (Slice 9) left `NULL`. A `NULL` here fails every charge-causing check closed, identically to "no provider customer found" — no new paid activity can occur until the Agency owner explicitly re-consents through the ordinary Slice 9 flow. Never a silent conversion, never a fabricated consent, never a stale `workspace` value left pointing at the wrong new Workspace. See Implementation Contract 10 §4/§5 for the full corrected step order and design. |
