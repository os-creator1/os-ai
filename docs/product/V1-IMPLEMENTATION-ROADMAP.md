...
- **Code domains:** `app/Enums/Usage/PayerType.php` (no change needed, already defined), `app/Library/Usage/EffectivePayer.php` (extended) + new `EffectivePayerResolver`, `BillingProfileManager` (`assignPayer()`/`assertBillingResponsibilityAuthority()`), `UsageBillingCheckoutManager::initiateCharge()`, `UsageWalletManager::isWorkspacePaid()` and its five consumers, `PaymentInstrumentManager`'s duplicate consent check (deleted), RFC-005 §16 consent table (doc update alongside code).
