...
| Create/manage Client Workspaces | Provision a client, see it in Clients list | Agency | BW (relationship) | Agency Workspace owner only (Addendum §2) | Relationship lifecycle | No (provisioning itself) | Client cannot remove the managing relationship itself (Addendum §2) | Owner provisions a client and it appears with correct status in Clients | Not yet implemented (rows 2, 23) |
