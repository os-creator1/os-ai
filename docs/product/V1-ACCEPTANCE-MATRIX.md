...
| Capability | Expected outcome | Entitlement | Scope | Permission boundary | Lifecycle states | Paid? | Critical failure behavior | V1 acceptance statement | Current-main status |
|---|---|---|---|---|---|---|---|---|---|
| Use Agency surfaces per role | Reach Outreach/Clients list per own permissions | Agency | BW | Ordinary Admin/Staff role (§26) — **never** inherits Agency management authority merely from membership (Addendum §2, Blueprint §2) | — | Varies | A team member must never gain View As or AgencyRebill authority without being the Workspace owner (Addendum §2, §10) | An Agency Staff member can work an Outreach campaign but cannot View As a client or touch AgencyRebill settings | Not verified in this pass |
...
| See platform billing/revenue | Aggregate view across all three money lanes | Platform | Global | Platform Administrator | — | No (view) | Lanes A/B/C must never be mixed in reporting (Addendum §12) | Revenue dashboard separates SaaS, Business-customer, and Agency-client-subscription lanes | Not verified in this pass |
