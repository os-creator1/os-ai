...
| Capability | Expected outcome | Entitlement | Scope | Permission boundary | Lifecycle states | Paid? | Critical failure behavior | V1 acceptance statement | Current-main status |
|---|---|---|---|---|---|---|---|---|---|
| Use Agency surfaces per role | Reach Clients list, View As, Outreach, and ordinary non-financial client management per own Agency-Workspace permissions | Agency | BW | Ordinary Admin/Staff role (§26) with Agency-management permissions, gated on an active relationship (Addendum §2) — **View As is not owner-only**; only AgencyRebill consent/payer/funding configuration and relationship termination stay owner-only (Addendum §2, §10) | — | Varies (View As itself: no; funding-related sub-actions: yes) | A team member must never gain AgencyRebill consent/payer authority or the ability to terminate the relationship without being the Workspace owner; View As authority must never be inferred merely from ordinary membership in the *Client* Workspace (Addendum §2) | An Agency Staff member with Agency-management permissions can View As a linked client and work an Outreach campaign, but cannot touch AgencyRebill settings or end the relationship | Not verified in this pass |
