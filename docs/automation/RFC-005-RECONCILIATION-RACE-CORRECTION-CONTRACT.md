...
**Exactly 3 new methods, added to the same file, reusing its existing fixture helpers (`businessWithProviderCustomer()`, `registerVerifiedCheckoutOutcome()`, `markStuck()`) verbatim — no new fixture helper is required. Corrected across both correction rounds (Round 1 record items 3-5; test 2 further redesigned by Round 2 record item 2):**
...
**Required new imports for the test file, corrected in Correction Round 2:** `Illuminate\Database\UniqueConstraintViolationException` (test 2's own assertion that the raced attempt's ledger stays single-credited, and the type the job's own catch targets); `Illuminate\Support\Facades\Queue` (test 2's `Queue::fake()`/`assertPushed`); `App\Jobs\Usage\SendReceiptNotification` (test 2's assertion target); `App\Exceptions\Usage\UsageWalletNotFoundException` (test 3); `App\Repositories\Eloquent\EloquentBusinessFundingAttemptRepository` (test 2's own real, underlying repository instance, resolved from the container before the partial mock is installed); `Mockery` (test 2's `Mockery::mock($realRepository)` partial mock, available globally via the base test case, no explicit `use` required). `App\Library\Usage\UsageWalletManager` is not required by test 2 (no direct `creditFromFunding()` pre-seed is used, in either round's design) but remains imported for other purposes in the file if already present; `App\Repositories\Contracts\BusinessFundingAttemptRepository`, `App\Library\Usage\UsageBillingCheckoutManager`, and `Illuminate\Support\Facades\Event` are already imported.
