...
A resumed implementation attempt on `agent/rfc-005-m3` (base `fde693ba3474a975692b7d0ab48643045773a318`, this contract's own merged commit) stopped before writing `resources/views/customer/business/usage-billing/show.blade.php`, having discovered that §3.7 and §18 below both required editing that file, and that no numbered item in the original §23 allowlist authorized it — a genuine drafting gap, not new scope. This correction round performed a full prose-to-allowlist audit (not just that one file) and found **eight further paths** the original §23 allowlist omitted despite this contract's own prose requiring them, plus **one internally contradictory schema claim** (§12's `currencies.decimal_places` reference — no such column exists, confirmed by direct migration read) and **one factually incorrect claim** (§3.7/§18's assertion that `UsageBillingController.php` needs modification — direct code read of `show()` proves it does not, since the controller performs zero manual view-data composition and already fully delegates to `UsageBillingPresenter`).

**Findings, in full:**
1. `resources/views/customer/business/usage-billing/show.blade.php` — required by §3.7/§18, omitted from §23. **Added as item 104.**
2. `app/Library/Usage/UsageBillingPresenter.php` — the file that must actually gain the M3 view-data (not the controller, corrected evidence below), omitted from §23. **Added as item 105.**
3. `app/Library/Usage/UsageBillingDashboardViewModel.php` — the readonly DTO the presenter returns, must gain four new properties, omitted from §23. **Added as item 106.**
4. `routes/customer.php` — the five new customer-facing routes (§18, items 58–60) can only exist by being registered here; omitted from §23. **Added as item 107**, mirroring the identical gap the M2 contract's own Correction Round 1 already found and fixed for this same file.
5. `routes/public.php` — the Stripe webhook route (item 61) cannot be registered in `routes/customer.php` (that whole file sits behind `RouteServiceProvider`'s `auth` middleware group, confirmed by direct read — a webhook caller can never authenticate) nor was any other route file allowlisted; omitted from §23. **Added as item 108.**
6. `routes/admin.php` — the admin provider-event routes (item 62) were never allowlisted anywhere; omitted from §23. **Added as item 109.**
7. `app/Console/Kernel.php` — §14/§11 item 10 describe `PurgeExpiredWebhookPayloads` and `ReconcileProviderPendingState` as periodic jobs, but neither is dispatched by any event, controller, or manager (unlike `ProcessPaymentProviderEvent`/`EvaluateBusinessAutoRecharge`, which are) — without a scheduler entry both are permanently unreachable dead code; omitted from §23. **Added as item 110.**
8. `tests/Feature/Usage/NoFakePaymentControlsRenderedTest.php` — directly asserts the M2 placeholder text and the *absence* of "Top up"/"Auto-recharge"/etc. controls; this assertion is obsolete the moment real M3 controls legitimately render. **Added as item 111 (modified).**
9. `tests/Feature/Usage/UsageBillingDashboardStripeIntegrationTest.php` — no existing or original-allowlist test proves the dashboard actually renders the M3 sections, that the placeholder is removed only where real functionality replaces it, or that the new partial is reachable rather than dead code. **Added as item 112 (new).**

**Two false/contradictory claims corrected (§3.7, §18, §12) — no path count impact:**
- §3.7 and §18 both claimed "M3 extends `UsageBillingController.php`." Direct read of the file (175 lines) shows `show()` is 12 lines that do nothing but call `$this->presenter->buildDashboardViewModel($business)` and pass the result to the view — zero manual view-data composition. **No change to `UsageBillingController.php` is required or authorized; both sections are corrected to attribute the change to `UsageBillingPresenter.php`/`UsageBillingDashboardViewModel.php` instead.**
- §12 claimed the currency-exponent divisor is "derived from `currencies.decimal_places`." Direct read of `database/migrations/2018_12_01_191808_create_currencies_table.php` and `app/Models/Currency.php` confirms this column has never existed in this repository. §12 is corrected to an explicit, provider-authoritative, fail-closed exponent map (§12 below) — no database column invented, no float arithmetic, no silent two-decimal assumption for every currency.

**One existing-item description widened to match reality, no path count impact:** item 100 (`UsageWalletManager.php`) originally said "exactly the `EvaluateBusinessAutoRecharge::dispatch()` addition." A resumed implementation attempt found this file must also gain `creditFromFunding()` and `configureAutoRecharge()` (both already anticipated in prose elsewhere in this same contract — §11 item 7, §15, RFC-005 §25's own ledger-write-authority assignment to `UsageWalletManager`) — a legitimate deepening of an already-authorized file, not a new path, but item 100's own description was incomplete. Corrected in §23 below.

**Existing M2 dashboard tests, individually inspected — only one is affected.** Direct read of all five existing `tests/Feature/Usage/UsageBilling*`/`NoFakePaymentControlsRenderedTest`/`tests/Unit/Usage/UsageBillingEnumsTest` files:
- `NoFakePaymentControlsRenderedTest.php` — asserts the exact M2 placeholder text and the *absence* of "Top up"/"Auto-recharge"/etc. controls. **Affected** — item 111 above.
- `UsageBillingDashboardViewDataTest.php` — five tests, all about wallet/debt/suspended/no-ledger-activity/no-spend-cap states; zero reference to the placeholder or the payment-methods card. **Unaffected**, confirmed by direct grep (`Payment|payment|placeholder|not yet configured` → zero matches) — not added to the allowlist.
- `UsageBillingDashboardAuthorizationTest.php` — the full actor/action matrix, asserting only HTTP status (`assertOk`/`assertNotFound`/`assertUnauthorized`) and `flash_success`/`flash_error` session keys; zero assertion on rendered payment-method content. **Unaffected** — not added to the allowlist.
- `UsageBillingLedgerPaginationTest.php`, `tests/Unit/Usage/UsageBillingEnumsTest.php` — ledger pagination and M1/M2 enum cases respectively; zero overlap with the payment-methods card. **Unaffected**, confirmed by direct grep — not added to the allowlist.

Neither correction expands M3's production scope beyond what §6/§18 already described in prose, invents a financial default, or authorizes resumed implementation — a separate, explicit human instruction remains required before implementation may resume. **This is Correction Round 1 of the maximum 2 correction rounds this contract permits** (`maximum_correction_rounds: 2`, unchanged, identical discipline to every prior RFC-004/RFC-005 contract in this repository).
...
- **Exact conversion** — `§10`'s already-defined `bcRoundHalfUp()` (BCMath, non-negative magnitudes, half-up), applied identically to M3's own outbound-amount conversion: `bcRoundHalfUp($amount_micro, '10000', 0)` for a currency with a 2-decimal exponent (USD), generalized to the wallet's actual currency exponent via the map below — **never hard-coded to USD's own 10,000 divisor** for a currency resolving to a different exponent.
- **Corrected, Correction Round 1 — internally contradictory schema claim removed.** The original text here derived the exponent from `currencies.decimal_places`. Direct read of `database/migrations/2018_12_01_191808_create_currencies_table.php` and `app/Models/Currency.php` (Correction Round 1 audit) confirms this column has never existed in this repository (`id, uid, user_id, name, code, format, status, timestamps` only). No M3 code may reference it, and no M3 code may invent it as a new migration (that is a schema change no allowlist item authorizes).
- **Provider-authoritative exponent map (Correction Round 1 resolution)** — the exponent is resolved from `currencies.code` (a column that does exist) through an explicit, static, three-tier map matching Stripe's own currently-published currency-decimal documentation, re-confirmed at implementation time, implemented inside the already-allowlisted gateway/manager code (items 39/42 — no new path): zero-decimal currencies (Stripe's documented list, e.g. `JPY`, `KRW`, `VND`) → exponent `0`; three-decimal currencies (Stripe's documented list, e.g. `BHD`, `KWD`, `OMR`) → exponent `3`; every other currency Stripe supports → exponent `2`. This is a provider-authoritative mechanism, not an invented default — it is Stripe's own published rule encoded as an explicit map, not a database column.
- **Fail-closed for an unrecognized code** — a `currencies.code` not resolvable through the three-tier map above (e.g., a custom/legacy code Stripe does not support) causes the outbound charge attempt to be refused via `ProviderInvalidRequestException` (already-allowlisted item 44) before any provider call — never a silent 2-decimal guess.
...
- **M3's own test-mode scope** — every M3 test exercises `USD` only (2-decimal tier); the fail-closed mechanism and the 0-/3-decimal tiers exist in code unconditionally but are not exercised by any M3 test — an honest scope boundary, not a claim of exhaustive currency test coverage.
...
Extends the existing `resources/views/customer/business/usage-billing/show.blade.php` (M2, item 104) with real M3 functionality. **Corrected, Correction Round 1:** the underlying data this view renders is supplied by `UsageBillingPresenter.php`/`UsageBillingDashboardViewModel.php` (items 105–106), never by `UsageBillingController.php` (unmodified — §3.7 evidence above; the controller's `show()` already fully delegates to the presenter, so no controller change is needed to add new presenter-supplied fields to the same existing `$dashboard` view variable).
...
- **Manual top-up** — an amount-entry form (major-unit display, e.g. `"$25.00"`, converted to micro-units server-side per §12), submitting to the new, dedicated `UsageBillingTopUpController::initiate()` action (item 59) — **never added to the existing `UsageBillingController.php`** (corrected, Correction Round 1 — the original "`UsageBillingController::initiateTopUp()`-shaped" phrasing was ambiguous and is replaced here with the exact, already-implemented class).
- **Auto-recharge configuration** — enable/disable toggle, threshold, amount, monthly cap — three-field form, each required together when enabling (manager-enforced, matching `UpdateBusinessSpendCapRequest`'s own M2 validation-class precedent), submitting to the new, dedicated `UsageBillingAutoRechargeController::configure()` action (item 60) — likewise never added to `UsageBillingController.php`.
...
**Requirements:**
- **Exact routes, controllers, FormRequests, middleware, CSRF behavior, and presenter keys** — all new customer-facing routes (SetupIntent creation, top-up initiation, auto-recharge configuration, instrument detach) are registered in `routes/customer.php` (item 107, corrected — omitted from the original allowlist, §"Correction Round 1 record" above), inside the existing `workspaces.` prefix group, following the identical uid-addressable, 404-never-403 pattern `UsageBillingController` already establishes; the webhook route is registered in `routes/public.php` (item 108, corrected) — **not** `routes/customer.php`, which sits entirely behind `RouteServiceProvider`'s `auth` middleware group and can never serve an unauthenticated webhook caller — with no CSRF middleware (§3.9, §23 item 102, corrected cross-reference — the original text cited "item 85," which is a test file, not `VerifyCsrfToken.php`), no session-dependent behavior, no authenticated-user assumption; the admin provider-event routes are registered in `routes/admin.php` (item 109, corrected).

### 18.1 M3 dashboard view-data shape (Correction Round 1)

`UsageBillingPresenter::buildDashboardViewModel()` (item 105, modified — the same existing method, not a new method, not a second presenter) gains exactly three new constructor-injected read dependencies — `PaymentProviderCustomerRepository`, `BusinessPaymentInstrumentRepository`, `BusinessFundingAttemptRepository` (all three already new, already-allowlisted items 28/29/31) — and `UsageBillingDashboardViewModel` (item 106, modified) gains exactly four new readonly public properties, appended after the existing `ledger` property:

- **`public ?array $paymentMethod`** — shape `array{brand: ?string, last_four: ?string, expiry_month: ?int, expiry_year: ?int}|null`. Source: resolve the business's current `PaymentProviderCustomer` via `findActiveByBusinessId()` when `$payer['payer_type'] === 'business'`, or `findActiveByWorkspaceId()` when `$payer['payer_type'] === 'workspace'` (using the wallet's already-loaded `$payerAssignment`, no new lookup pattern); `null` when no payer is configured yet. Then `BusinessPaymentInstrumentRepository::findDefaultForProviderCustomer()`. `null` end-to-end when no provider customer or no default instrument exists. Never the raw `provider_payment_method_id`, never a client secret.
- **`public array $autoRecharge`** — shape `array{enabled: bool, threshold_micro: ?string, amount_micro: ?string, monthly_cap_micro: ?string, recharged_this_period_micro: string, consecutive_recharge_failures: int}`. Source: the SAME already-loaded `$wallet` Eloquent row — **no new repository call** — reading `auto_recharge_enabled`, `auto_recharge_threshold_micro`, `auto_recharge_amount_micro`, `monthly_recharge_cap_micro`, `recharged_this_period_micro`, `consecutive_recharge_failures` (all M1 columns). When `$wallet === null`, every field takes its M1 schema default (`enabled: false`, micro fields `null`, counters `0`/`0`).
- **`public LengthAwarePaginator $fundingHistory`** — paginated 25/page (matching the existing `$ledger` precedent, via `BusinessFundingAttemptRepository::query()` — the shared `BaseRepository::query()` method, already exposed, unmodified, the identical mechanism `$ledgerPage`'s own assembly already uses, avoiding any new repository method), filtered `where('business_id', ...)`, ordered `orderByDesc('created_at')->orderByDesc('id')`. Each row a **plain array** (no new presentation-row class — matching the `$walletView`/`$featureLimits`/`$payerView` precedent, since `business_funding_attempts` already carries safe, pre-formatted display columns), shaped `array{state: string, purpose: string, amount_micro: string, payment_method_display: string, created_at: CarbonImmutable, failure_reason: ?string}` — `state`/`purpose` are the enum's own `->value`; `amount_micro` is `expected_amount_micro`; `payment_method_display` is the already-safe `payment_method_display_snapshot` column (§21.3, e.g. `"visa •••• 4242, exp 12/26"`) — never the raw provider payload, never `provider_session_or_intent_reference`.
- **`public bool $providerConfigured`** — `true` only when `services.stripe.secret`, `services.stripe.key`, and `services.stripe.webhook.secret` are all non-empty and `services.stripe.mode` is `test` or `live` — a pure config read, no gateway instantiation, no provider network call, preserving the presenter's own existing "plain read-assembly service, no write authority" character. `false` otherwise.

No Blade view resolves a repository, manager, or gateway directly — every one of these four fields crosses the existing controller→presenter→view-model→view boundary exactly like every existing M2 field (§18 requirement, unchanged).

### 18.2 `show.blade.php` dashboard integration (Correction Round 1, item 104)

Exactly the existing "Payment methods" card (`id="usage-billing-payment-methods"`, M2 base) is modified, plus one new card added:

- **`$dashboard->providerConfigured === false`** — the M2 placeholder text is preserved verbatim ("Payment methods and top-ups are not yet configured.") — this is the same honest unavailable state M2 already rendered, not a section M3 "genuinely implements" for this environment.
- **`$dashboard->providerConfigured === true`** — the placeholder is removed from this card only, replaced with: (a) `@include('customer.business.usage-billing.partials.payment-method', ['dashboard' => $dashboard, 'workspaceUid' => $workspaceUid, 'businessUid' => $businessUid])` (item 67, unmodified — the new partial itself) rendering `$dashboard->paymentMethod` plus the Set up/Replace/Remove actions, posting to the new item-107 routes; (b) a top-up form (amount-entry, major-unit display, same client-side major-to-micro conversion pattern the existing spend-cap/feature-limit forms already use) posting to the new item-107 top-up route; (c) an auto-recharge form reading `$dashboard->autoRecharge`, posting to the new item-107 auto-recharge route.
- **New card** — `id="usage-billing-funding-history"`, added immediately after the existing `usage-billing-ledger` card (never replacing it), rendering `$dashboard->fundingHistory` with `{{ $dashboard->fundingHistory->links() }}` (identical pagination convention to the existing ledger card), an honest "No funding activity yet." empty state matching the ledger card's own established empty-state convention.
- **Everything else preserved byte-for-byte** — billing status, balance & spend, per-feature usage limits, payer, billing contact, recent usage activity/ledger, all flash/error rendering. This correction authorizes touching only the "Payment methods" card's inner content and adding one new card after the ledger card — nothing else in this file.
- **No live-mode UI, no fake success** — a "pending"/"processing" state renders honestly as such; success renders only once `state` is genuinely `succeeded`; no secret, client-secret, or raw payload rendered outside the Stripe.js-required `data-` attributes the existing M2 JS convention already establishes for other forms.
...
**Admin provider-event/reconciliation UI** — included, since RFC §30 explicitly assigns "review and disposition webhook events that have exhausted their claim/retry attempts" to the admin surface, and M3 is the milestone that first makes this operationally necessary (the claim/lease/disposition algorithm exists nowhere before M3). A minimal `Admin\PaymentProviderEventController` surface (item 62): list exhausted events, view non-sensitive fields (never the raw payload while still `failed`/`processing` — and never at all once purged), dispose with a mandatory note — registered in `routes/admin.php` (item 109, corrected, Correction Round 1) inside the existing `Route::middleware(EnsureUserIsAdministrator::class)->group(...)` block, matching `WorkspacePlanCatalogController`'s own established RFC-004 M3 precedent (no literal `admin/` URI segment, no `admin.` name prefix — both applied externally by `RouteServiceProvider`). No new `config/permissions.php` entry is added — that file is not allowlisted; the existing group-level `can:access backend` gate plus `EnsureUserIsAdministrator` is the sole authorization check, matching the same two-layer defense-in-depth the Workspace admin module already establishes without a per-action permission string.
...
- **Stripe CLI webhook-forwarding command** — `stripe listen --forward-to <local-url>/stripe/webhook/usage-billing` (corrected, Correction Round 1 — finalized against `routes/public.php`, item 108; the original placeholder path implied `routes/customer.php`, which sits behind `auth` middleware and cannot serve an unauthenticated webhook), and the exact resulting `whsec_...` value's env-var name (`STRIPE_WEBHOOK_SECRET`) it must be placed into for local forwarding to verify correctly.
...
### Tests (31 new — corrected, Correction Round 1; original text misstated "30")
...
100. `app/Library/Usage/UsageWalletManager.php` — **modified, not new**: the `EvaluateBusinessAutoRecharge::dispatch()` addition at the two existing negative-`available_delta_micro` ledger-insert sites (`reserve()`'s reservation insert, `commit()`'s overage-charge insert), each an additive one-line call plus its constructor-injection-free static dispatch (no new constructor dependency required — the job resolves `UsageWalletManager`/`PaymentInstrumentManager`/`UsageBillingCheckoutManager` itself via the container); **plus, corrected/widened, Correction Round 1** — two new public methods, `creditFromFunding(int $businessId, UsageLedgerEntryType $entryType, int $amountMicro, int $fundingAttemptId, string $correlationKey): void` (RFC §25's own ledger-write-authority assignment to `UsageWalletManager`; implements the exact debt-clearing formula §13 of the RFC already specifies) and `configureAutoRecharge(Business $business, bool $enabled, ?string $thresholdMicro, ?string $amountMicro, ?string $monthlyCapMicro, int $actorUserId): void` (§15 of this contract already anticipates this exact capability), plus a private `assertChargeCausingConsentForAutoRecharge()` mirroring the identical payer-consent check `PaymentInstrumentManager`/`UsageBillingCheckoutManager` each already carry their own copy of (§3.6's established duplicated-not-shared convention — no shared trait is introduced). No existing method signature, existing business logic, or existing M1/M2 test-covered behavior changes. §15.
...
102. `app/Http/Middleware/VerifyCsrfToken.php` — **modified, not new**: exactly one additive entry to the existing `$except` array for the new Stripe webhook route's exact URI pattern (§3.9, item 108). No existing `$except` entry is added, removed, reordered, or altered.
...
### Correction Round 1 additions (9 — items 104–112; see "Correction Round 1 record" above for the full audit)

**Modified production paths (7):**

104. `resources/views/customer/business/usage-billing/show.blade.php` — **modified, not new**: exactly the "Payment methods" card's inner content (§18.2), plus one new `usage-billing-funding-history` card added immediately after the existing ledger card. No other section changes. §18.2.
105. `app/Library/Usage/UsageBillingPresenter.php` — **modified, not new**: `buildDashboardViewModel()` gains three new constructor-injected read dependencies (`PaymentProviderCustomerRepository`, `BusinessPaymentInstrumentRepository`, `BusinessFundingAttemptRepository` — all three already-allowlisted items 28/29/31) and assembles four new fields (§18.1). No existing method signature, existing field, or existing M2 assembly logic changes.
106. `app/Library/Usage/UsageBillingDashboardViewModel.php` — **modified, not new**: exactly four new readonly public properties appended after the existing `ledger` property — `paymentMethod`, `autoRecharge`, `fundingHistory`, `providerConfigured` (§18.1 exact shapes). No existing property removed, renamed, or reordered.
107. `routes/customer.php` — **modified, not new**: exactly five new route definitions inside the existing `workspaces.` prefix group, immediately after the existing five M2 usage-billing routes — `POST .../usage-billing/payment-method/setup-intent` (`UsageBillingPaymentMethodController@createSetupIntent`), `POST .../usage-billing/payment-method/confirm` (`@confirmSetupIntent`), `POST .../usage-billing/payment-method/{instrument}/detach` (`@detachInstrument`), `POST .../usage-billing/top-up` (`UsageBillingTopUpController@initiate`), `POST .../usage-billing/auto-recharge` (`UsageBillingAutoRechargeController@configure`). No existing route definition in this file is added, removed, reordered, or altered — mirroring the M2 contract's own Correction Round 1 precedent for this identical file exactly.
108. `routes/public.php` — **modified, not new**: exactly one new route, `POST stripe/webhook/usage-billing` → `StripeWebhookController@handle`, named `webhooks.stripe.usage-billing`, added following the identical unauthenticated-webhook shape this file's own existing DLR routes already establish ("No middleware will not affect these routes"). No existing route in this file changes.
109. `routes/admin.php` — **modified, not new**: exactly two new routes inside the existing `Route::middleware(EnsureUserIsAdministrator::class)->group(...)` block — `GET provider-events` (`PaymentProviderEventController@index`), `POST provider-events/{event}/dispose` (`@dispose`) — no literal `admin/` segment, no `admin.` name prefix (both applied externally, matching `WorkspacePlanCatalogController`'s own RFC-004 M3 precedent exactly). No existing route in this file changes.
110. `app/Console/Kernel.php` — **modified, not new**: exactly two new `$schedule->job(...)` entries inside the existing non-demo-stage `else` branch — `$schedule->job(new PurgeExpiredWebhookPayloads())->hourly();` and `$schedule->job(new ReconcileProviderPendingState())->everyFiveMinutes();` — added immediately after the existing `opportunity:sweep-expired-snoozes` registration, the established precedent for adding a new scheduled entry to this file. No existing schedule entry changes.

**Modified/new test paths (2):**
...
20. **(Correction Round 1)** `grep -c "PurgeExpiredWebhookPayloads\|ReconcileProviderPendingState" app/Console/Kernel.php` → `2` (both jobs actually scheduled, proving neither is unreachable dead code).
...
5. Managers and state machines (items 42–43, including item 100's widened `creditFromFunding()`/`configureAutoRecharge()`, Correction Round 1).
...
**Correction Round 1's own verification (this correction round):**
- `git diff --check` — clean.
- `git diff --name-only` — exactly `docs/automation/RFC-005-M3-CONTRACT.md` (this file is tracked in this worktree, checked out from the merged base — not untracked).
- `git status --short` — exactly ` M docs/automation/RFC-005-M3-CONTRACT.md`.
- `git diff --cached --name-only` — empty before staging.
- Stage the one file by its exact path only (never `git add -A`/`git add .`).
- Commit exactly: `docs: correct RFC-005 M3 integration allowlist`.
- Push normally to `origin chore/rfc-005-m3-contract-correction-1`. No force push. Do not push `main`.
- If `gh` is available, open a PR into `main`. Otherwise report the exact GitHub comparison URL.
