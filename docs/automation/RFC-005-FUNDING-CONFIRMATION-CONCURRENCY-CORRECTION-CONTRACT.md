...
### 2.1 `confirmSucceeded()` — unified across every purpose
...
private function confirmSucceeded(BusinessFundingAttempt $attempt, TransitionSource $source, ?int $providerEventId, ?int $actorUserId = null, ?string $verifiedPaymentMethodDisplay = null): void
{
    if ($attempt->purpose === FundingAttemptPurpose::AddonPurchase) {
        ...
        try {
            ...
            // A concurrent/replayed caller already committed this exact
            // credit. Fall through to the same shared finalizer below —
            // it alone determines, under lock, whether this caller must
            // still perform the attempt's own terminal transition.
        }
    }

    $didFinalize = $this->finalizeFundingAttemptState($attempt, $source, $providerEventId, $actorUserId, $verifiedPaymentMethodDisplay);
...
**Crash recovery, explicit, per the review's own required explanation:** if execution stops after `finalizeAddonPurchaseIfPending()` returns (purchase durably `Completed`) but before `finalizeFundingAttemptState()`'s own transaction commits, the attempt's own persisted `state` remains non-terminal. A later replay re-enters `confirmSucceeded()`, calls `finalizeAddonPurchaseIfPending()` again — whose own top-of-method guard sees `status === Completed` and returns immediately, a genuine no-op, since fulfillment already happened — and then reaches `finalizeFundingAttemptState()` for the first time, which finds the attempt still non-terminal and safely performs the one remaining transition. Exactly one funding-attempt transition and one `BusinessFundingAttemptSucceeded` dispatch result, regardless of how many replays occur before or after the crash. This holds identically for both `wallet_credit` and `direct_deliverable`.
...
7. **A crash between financial credit and attempt-state finalization remains recoverable on replay.** The specific, additional defect §1 independently identified (today's code finalizes state *before* crediting, so a crash in that window is unrecoverable and silent). Reordering to credit-first, then always finalizing through the one shared, locked finalizer (§2.1, §2.2) — never a separate unlocked path — is what makes the *replay* path, whichever of the same three already-existing callers performs it, safely re-enter that same finalizer and complete the one remaining transition, exactly once, regardless of whether the earlier, crashed attempt was itself the credit's winner or loser.
...
2. **`test_the_credit_winner_safely_defers_when_the_recovery_caller_finalizes_first`** — the deterministic proof of the exact interleaving the review identified, added in Correction Round 1: caller A's own credit succeeds; **before** A reaches its own finalization, caller B's credit attempt collides and B finalizes first; A then resumes and must safely defer. Mechanically constructed as a single-process interception on `BusinessFundingAttemptRepository::findForUpdateById()` — the first statement inside the shared `finalizeFundingAttemptState()` — using the same constructor-initialized Mockery partial-mock technique already established for PR #143's own test 2 (`Mockery::mock(EloquentBusinessFundingAttemptRepository::class, [new BusinessFundingAttempt()])->makePartial()`, bound as `BusinessFundingAttemptRepository::class`). Creates one stuck-eligible ManualTopUp attempt (`$racedAttemptId`) with a registered verified checkout outcome. Resolves `$realRepository = app(BusinessFundingAttemptRepository::class)` before installing the mock. Stubs `findForUpdateById()` **with `->with($racedAttemptId)->once()`**, so it fires exactly once — precisely when caller A's own call to `finalizeFundingAttemptState()` reaches its own lock acquisition. Inside that single interception, in order: (a) fetches `$loser = $realRepository->findById($racedAttemptId)` (a second, independent, still-`ProviderPending` snapshot); (b) runs caller B's own **entire** confirmation to completion — `app(UsageBillingCheckoutManager::class)->confirmAttemptFromReturn($loser)` — which internally collides on its own credit attempt (A's own credit, from before this interception fired, already committed), falls through to `finalizeFundingAttemptState()`, and reaches its own `findForUpdateById($racedAttemptId)` call: since the specific, `->once()`-scoped expectation is already consumed by A's own still-in-progress outer call, this **nested** call correctly falls through to a second, `withAnyArgs()`-scoped expectation delegating to `$realRepository`, so B genuinely acquires the real lock, finds the attempt not yet terminal, finalizes it for real, and dispatches; (c) only then, as the closure's own final action, calls `$realRepository->findForUpdateById($racedAttemptId)` itself, returning that (now-`Succeeded`) row to caller A's own, still-paused, outer call. `Event::fake([BusinessFundingAttemptSucceeded::class])`, `Queue::fake()`. Calls `$checkoutManager->confirmAttemptFromReturn($winner)` for caller A — a single call, entirely self-contained; A's own credit succeeds for real before the interception fires, and A's own resumed `finalizeFundingAttemptState()` call sees the row B already finalized and returns `false`, so A performs no mutation and no dispatch. Asserts: the call does not throw; exactly one `business_usage_ledger_entries` row; **exactly one** `business_funding_attempt_transitions` row with `to_state = 'succeeded'`; `Event::assertDispatchedTimes(BusinessFundingAttemptSucceeded::class, 1)`; `Queue::assertPushed(SendReceiptNotification::class, 1)`; the attempt's final persisted state is `Succeeded`. This is the test that would have failed against the initial draft's own unlocked winner path and passes only under the corrected, uniformly-locked design.
...
| # | Path | Status | Reason |
|---|---|---|---|
| 1 | `tests/Feature/Usage/FundingConfirmationConcurrencyCorrectionTest.php` | REQUIRED (new file) | 5 methods (§5.1, one added in Correction Round 1 — the deterministic winner-defers-to-recovery-caller interleaving), proving guarantees 1, 3, 4, 5, 6, 7 directly and deterministically, in-process. |
...
- `php artisan test tests/Feature/Usage/FundingConfirmationConcurrencyCorrectionTest.php` — the new file itself; expected 5 methods, all passing.
...
**No genuine open human decision was identified during this audit.** Every design choice in §2 was derivable mechanically from the ten stated guarantees and the current, directly-read code and schema — none required a judgment call this contract deferred rather than made. Correction Round 1's own two findings were confirmed defects, not design disagreements — both are now closed (§2's own "Why this closes..." notes). Two smaller design choices remain, unaffected by this round, that a human reviewer could still reasonably disagree with (applying the lock uniformly to `direct_deliverable`, §2.2/§3; or wrapping the two-statement finalization in its own local transaction, §2.1/§3, as a bundled strengthening beyond the letter of guarantee 7) — either would be a correction-round-scoped disagreement to raise against this document, not an unresolved question this document itself leaves open.
