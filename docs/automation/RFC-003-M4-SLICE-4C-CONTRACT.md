...
**This contract is a proposal, not a lease.** Implementation is not authorized, and no implementation PR or branch may be opened, until a human reviews and merges this contract together with a separate, human-reviewed `docs/automation/AI-AUTONOMY-STATE.json` update that explicitly sets `implementation_authorized: true` and pins the exact target PR number, head branch, and full starting SHA, under the manual bounded-contract workflow described in `docs/automation/AI-SUBSCRIPTION-LOOP.md`'s manual completion path. That authorizing update is a human-reviewed and human-merged change, not an automatic-start trigger; `start_automatically_after_contract_merge` remains `false` throughout. Until that separate authorizing merge happens, this document describes proposed scope only.
