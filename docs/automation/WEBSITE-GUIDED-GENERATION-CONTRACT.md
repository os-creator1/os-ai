...
| `differentiators`, `ideal_customers`, `customer_problems`, `brand_voice`, `primary_conversion_goal`, `conversion_target`, `vertical_key`, `growth_priority_service_ids`, `growth_priority_location_ids`, `prohibited_claims` | 180 | identity/direction facts, moderate change rate. **Correction (Slice 1 final correction pass):** `prohibited_claims` was omitted from this table entirely in the prior draft; it is placed in this group because it is exactly this kind of fact — it shapes generation direction rather than asserting a checkable operational or high-stakes claim. |
...
**Correction (identity design, locked):** `key` is a **stable pack-family identifier only** — it never embeds a version number. The version lives exclusively in the separate `version` column. `general` is a contracted literal family key (not an illustrative example) for the fallback family described in §6.3 step 3; the general-fallback resolution step matches it by identity (`key = 'general'`) **and** by shape (both `applies_to_*` columns null) — both must agree, which is itself a write-time invariant (§6.3's determinism rule) rather than two independent checks that could disagree.

| Column | Type | Notes |
|---|---|---|
| `id` | bigint PK | |
| `key` | string(40) | a stable family identifier, never version-suffixed — e.g. `general`, `home_services`, `roofing`, `photobooth`. The contracted general-fallback family key is exactly `general`. |
...
| `version` | unsigned int | packs are immutable once referenced by any completed generation; a new question or changed wording ships as a new `version` row under the same `key`, never an in-place edit — mirrors `WebsiteRevision`'s own immutability discipline. `(key, version)` is unique; `key` alone is **not** unique, since the same family accumulates multiple version rows over time. |
...
Every `field_key` in a pack's `questions` array is validated at write time against `BusinessKnowledgeProfileFieldKey` (§5.1) — a question pack can only ever collect facts the Profile already knows how to store. **Correction (validation boundary, honest statement):** this and every other shape rule in this section is enforced by `QuestionPack`'s Eloquent model-event hook (`saving`), which protects every write that goes through `QuestionPack::create()`/`update()`/`save()` — including this contract's own fixtures and any future seeder or admin surface that uses the model. It does **not** and cannot protect a hypothetical raw `DB::table('question_packs')->insert(...)` call, which bypasses Eloquent entirely; no such call exists anywhere in this contract's implementation, and none is authorized. The same honest boundary applies to `BusinessVertical`.

### 6.3 Pack resolution order — locked, corrected for confirmed-vertical and null-industry safety

`BusinessKnowledgeProfileManager::completenessCheck()` resolves exactly one pack, in this order:

1. **Exact vertical** — reachable only when the Profile's `vertical_key` is **both** (a) tracked as `customer_confirmed` in `business_knowledge_profile_field_states` for the `vertical_key` field (§5.2 — an `unverified` vertical_key, e.g. one written by an import with `markVerified: false`, never reaches this step) **and** (b) still an active row in `business_verticals` (an operator may deactivate a vertical after a customer confirmed it; a deactivated vertical is treated the same as none). When both hold, resolve the highest-`version`, `is_active = true` pack where `applies_to_vertical_key` equals that key. If no such pack exists, fall through to step 2 — the absence of a vertical-specific pack is not itself a resolution failure.
2. **Broad industry** — reachable only when `$business->industry` is non-null. This step is **skipped entirely** when industry is null; it never falls through to a `whereNull('applies_to_industry')` match, since that shape belongs exclusively to step 3's general fallback. When industry is non-null, resolve the highest-`version`, `is_active = true` pack where `applies_to_industry` equals `$business->industry->value` **and** `applies_to_vertical_key IS NULL` (explicit, not inferred from the industry column alone) — a pack can never have both columns non-null (§6.2), so this second condition only guards against a future query refactor accidentally dropping the industry filter and matching a vertical-targeted row.
3. **General fallback** — the highest-`version`, `is_active = true` pack with `key = 'general'` **and** both `applies_to_industry`/`applies_to_vertical_key` null (§6.2's identity-plus-shape agreement).

Each step orders candidates by `version` desc, then `id` desc as a deterministic tie-break, so a resolution call's result never depends on database row-retrieval order even in the presence of a catalog-authoring mistake.

**Determinism at the catalog-write boundary (corrected):** because `applies_to_industry`/`applies_to_vertical_key` are nullable columns, no portable database constraint can by itself guarantee "at most one active pack family per target scope" (mirrors the existing at-most-one-non-null-column limitation already documented in the `question_packs` migration). This contract therefore requires `QuestionPack`'s model-event validation (§6.2) to reject saving an active pack whose target scope (`applies_to_vertical_key`, or `applies_to_industry` when vertical is null, or the general shape) is already actively served by a **different** `key` family — two or more active versions of the *same* family for the same scope remain fully supported (that is the immutable-versioning model itself), but two different families racing for the same scope is rejected at write time, not discovered later as a silent "wrong pack picked" bug.
...
- **Roofing** (§22 decision 3, locked: stays under the existing `BusinessIndustry::HomeServices` case **plus** a confirmed `vertical_key = 'roofing'` — no new `BusinessIndustry` case is ever added): a `roofing` question-pack family (`applies_to_vertical_key = 'roofing'`) targets `pricing_method`/`financing_available` separately (§9), `credentials` (license number, insurance), `offers` (repair/replacement/inspection as separate offer entries), `primary_conversion_goal` defaulting to `quote_request`, and location `hours` including an emergency-availability note in the `notes` field (§5.5) rather than inventing a new column.
...
### 6.6 Question-pack UI rendering status — honest disclosure

**Correction:** Slice 2's completeness/edit views render a resolved pack's `questions[].prompt` text as a **plain, read-only preview list** (`show.blade.php`) — they do **not** dynamically render an input control per `input_type`/`options` (e.g. an actual `<select>` for `select`/`multi_select`, or a checkbox for `boolean`). The generic, field-keyed form in `edit.blade.php` is what a customer actually fills in today, regardless of which pack resolved. Making the edit form itself pack-driven — rendering the exact control `input_type` calls for, using `options` where present, and asking only the pack's chosen subset of fields in the pack's own order — is explicitly deferred to a future, not-yet-numbered slice; it is not implementation-authorized by Slice 1 or Slice 2 of this contract. `QuestionInputType`/`options` are validated and stored correctly today (§6.2) so that future slice has a well-formed shape to render against, but nothing in Slice 1-2 claims that rendering exists yet.
...
|---|---|---|---|
| **Slice 1 — Business Knowledge Profile foundation** | **5 migrations** (`create_business_knowledge_profiles_table`, `add_hours_and_provenance_to_business_locations_table`, `create_business_knowledge_profile_field_states_table`, `create_business_knowledge_profile_changes_table`, `backfill_business_knowledge_profiles_for_existing_businesses`); `BusinessKnowledgeProfile`/`BusinessKnowledgeProfileFieldState`/`BusinessKnowledgeProfileChange` models; `BusinessKnowledgeProfileManager` (§4.3, §5.5) including `updateLocationHours()` and a genuinely read-only `completenessCheck()`; `BusinessKnowledgeProfileFieldKey` enum (§5.1); the two contracted enum-backed fields' actual PHP enums (§4.2) — `BusinessPricingMethod` and `BusinessPrimaryConversionGoal`; the `BusinessKnowledgeProfileCompleteness` read-only DTO (§4.3); the §3.4 page-count-ceiling fix inside `WebsiteDraftPageService::createPage()`. **Correction: `business_media_assets` is not part of Slice 1 — it is Slice 5, §13.** **Correction (Slice 1 final correction pass):** the merged contract's Slice 1 allowlist mechanically omitted two paths this scope genuinely requires — `app/Models/BusinessLocation.php` (fillable/cast declarations for the new hours/provenance columns only; no other change to this file) and `app/Library/Business/BusinessKnowledgeProfileCompleteness.php` (the DTO §4.3 names but never itself gives a path) — both are added to this allowlist below, alongside the two new enum paths. | none (pure additive schema + two narrow existing-file extensions) | `database/migrations/*business_knowledge_profile*`, `database/migrations/*add_hours*business_locations*`, `app/Models/BusinessKnowledgeProfile*.php`, `app/Library/Business/BusinessKnowledgeProfileManager.php`, `app/Library/Business/BusinessKnowledgeProfileCompleteness.php`, `app/Enums/Business/BusinessKnowledgeProfileFieldKey.php`, `app/Enums/Business/BusinessPricingMethod.php`, `app/Enums/Business/BusinessPrimaryConversionGoal.php`, `app/Library/Website/WebsiteDraftPageService.php` (page-count check only), `app/Models/BusinessLocation.php` (hours/provenance fillable/cast additions only) |
| **Slice 2 — Verticals + question packs + completeness UI** | `business_verticals` and `question_packs` tables/models (§6); `completenessCheck()` extended to also resolve the applicable question pack (§6.3); the `vertical_key` write path in `updateFields()` validated against the now-existing catalog; the guided-setup controller actions/views (Option A, §3.3, locked). **Correction (Slice 2 implementation pass):** the merged contract's Slice 2 allowlist column mechanically omitted the model/enum paths this scope genuinely requires and the two Slice-1-owned files `completenessCheck()`/`updateFields()` actually live in -- `app/Models/BusinessVertical.php`, `app/Models/QuestionPack.php`, `app/Enums/Business/QuestionInputType.php` (the question pack's `input_type` column is contracted as enum-backed, §6.2), and `app/Library/Business/BusinessKnowledgeProfileManager.php`/`app/Library/Business/BusinessKnowledgeProfileCompleteness.php` (already Slice-1-owned; extended here, never re-created) -- all five are added to this allowlist below. No production catalog content (real `business_verticals`/`question_packs` rows) is seeded by this slice -- §6.4 already documents worked-example content as operator-authored, never contract-seeded; this correction only adds the file paths the contract's own named scope requires. | Slice 1 | `database/migrations/*business_verticals*`, `database/migrations/*question_packs*`, `app/Models/BusinessVertical.php`, `app/Models/QuestionPack.php`, `app/Enums/Business/QuestionInputType.php`, `app/Library/Business/BusinessKnowledgeProfileManager.php` (vertical_key validation + question-pack resolution only), `app/Library/Business/BusinessKnowledgeProfileCompleteness.php` (adds the resolved `questionPack` property only), new controller under `app/Http/Controllers/Customer/Business/`, new views under `resources/views/customer/business/website/` |
...
No slice touches `app/Http/Controllers/Public/WebsiteController.php`, `app/Library/Website/WebsitePublisher.php`, `app/Library/Website/WebsiteSnapshotBuilder.php`, `app/Library/Website/WebsitePublicEntitlementGate.php`, `app/Enums/Website/WebsiteSectionType.php`'s case list, or any B3/B4/B5/GBP file. `app/Library/Website/WebsiteUrlRules.php` (Slice 4), `app/Library/Website/WebsiteDraftPageService.php` (Slice 1, page-count check only), and `app/Models/BusinessLocation.php` (Slice 1, hours/provenance fillable/cast additions only) are the three narrow, cited exceptions to "no other Slice A file changes."

---

## 17. ACCEPTANCE CRITERIA

1. **Corrected — split by catalog-activation state, since Slices 1-2 seed no production catalog rows (§6.4, §23 item 3):**
   - **Before an operator activates any real `business_verticals`/`question_packs` catalog content:** a Business with an empty Knowledge Profile completes `completenessCheck()` and correctly resolves no pack (`questionPack === null`, §6.3), and the completeness UI falls back to the generic, always-available `FIELD_COPY` labels/help text (§16 Slice 2) rather than throwing or silently showing nothing — "no catalog seeded yet" is a supported, tested state, not a gap.
   - **After an operator activates appropriate catalog content:** a Business with an empty Knowledge Profile completes `completenessCheck()` and receives exactly the vertical/industry-appropriate question set (§6.3's resolution order), never a generic unbounded prompt.
   - **Deployment prerequisite (explicit, not invented editorial content):** the "always receives an appropriate question set" behavior is a property of an operator having seeded at least a `general`-family pack (and any vertical/industry packs desired) — it is not automatically true the moment this contract's code ships. This contract's slices supply the schema, validation, and resolution logic only (§6.5, §23 item 3); seeding real catalog rows remains a separate, human/operator-run task before the "always appropriate" guarantee holds in production.
...
- **Vertical/question-pack resolution**: an active, **customer-confirmed** vertical-targeted pack wins over a broad-industry pack, which wins over the `general` fallback; an unconfirmed or inactive vertical_key never reaches the exact-vertical step; a null industry never matches the general-shaped fallback row; an inactive/older-version pack is never selected; adding a new vertical requires zero PHP/migration changes (a pure-data test).
...
3. **Whether to seed real `business_verticals`/`question_packs`/`website_templates` catalog *content*** (the actual question wording, vertical list, template copy) is an editorial/operational task for whoever runs Slices 2-3, not a decision this contract makes — it only fixes the schema and resolution rules those seeds must conform to. **Explicit deployment prerequisite:** until at least a `general`-family `question_packs` row is seeded and activated, `completenessCheck()` correctly resolves no pack for every Business (§17 item 1, corrected) — this is expected, tested behavior for an unseeded environment, not a defect, but it does mean the "always get an appropriate question set" guarantee only holds in an environment where an operator has completed this seeding step.
4. **Whether/when to implement a pack-driven dynamic edit form** (rendering the exact `input_type`/`options`-selected control per question, §6.6) rather than today's generic fixed-field form is not decided by this contract — it is deferred to a future, not-yet-numbered slice.
