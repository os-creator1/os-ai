...
- **`app/Http/Requests/Admin/MarkHotLeadCalledRequest.php`** — `authorize()`: `$this->user()->can('chat_box')`. `rules()`: `'id' => 'required|integer|exists:chat_boxes,id'`. Existence is checked here (a cheap, standard Laravel rule); **ownership** (§8) is enforced separately in the controller's scoped update, not duplicated in this rule, since `exists:` alone cannot express the tenant boundary and must not be asked to.
...
`AiAnalyticsController::markBooked($id)` takes its only input from the route's own `{id}` URL segment (no request body field) — validated by Laravel's own implicit route-parameter binding plus the controller's scoped-update-then-404 pattern (§8); no dedicated `FormRequest` is required or authorized for it, consistent with "prefer reuse of existing infrastructure" over adding a file with no genuine validation surface to guard.
...
|---|---|---|
| Unauthenticated request to any of the 6 routes | Laravel's standard `auth` middleware redirect-to-login (302) | Framework default, unchanged from every other authenticated route in this repository |
| Authenticated, lacks the broad `access_backend`/`access backend` gate | Route-level `can:` middleware throws `AuthorizationException` before controller code runs → Laravel's default 403 | Framework default, matches every other `can:`-gated route |
| Authenticated, lacks the specific `chat_box`/`manage ai_settings` permission | Controller's `$this->authorize()` throws `AuthorizationException` → 403 | Matches `ChatBoxController`/`SettingsController::aiSettings()`'s own existing behavior |
| Actor targets another tenant's `chat_boxes` row by ID (mark-called/mark-booked) | `abort(404)` | §8 — indistinguishable from "does not exist," satisfying the no-existence-leak requirement |
| Nonexistent record | `abort(404)` | Same path as above — deliberately indistinguishable |
| Invalid or unauthorized `campaign_id` filter | Silently ignored; safe unfiltered-but-tenant-scoped fallback | §9 |
| Malformed write input (missing `id`, missing `system_prompt`/`model`) | Standard Laravel `FormRequest` validation failure → 302 redirect-back with `$errors` | Matches every existing `FormRequest` in this repository (e.g. `SentRequest`) |

No new response shape, status code, or error-page convention is invented anywhere in this table.
...
1. **`tests/Feature/Security/HotLeadsSecurityTest.php`** — a guest (no authenticated user) requesting `GET /admin/hot-leads` or posting `POST /admin/hot-leads/mark-called` is redirected to login, not 200; an authenticated customer lacking `chat_box` receives 403; an authenticated customer holding `chat_box` receives 200 and sees only `chat_boxes` rows where `user_id` matches their own account, proven by seeding two distinct tenants' rows and asserting tenant A's response never contains tenant B's data; tenant A posting `mark-called` with tenant B's row ID leaves that row's `called` column unchanged and receives 404; tenant A posting with its own row ID successfully sets `called = 1` and redirects (the legitimate-actor path must still work, proving no over-restriction); posting a nonexistent ID also receives 404, indistinguishable in response shape from the cross-tenant case (proving no existence leak).
2. **`tests/Feature/Security/AiAnalyticsSecurityTest.php`** — identical guest/unauthorized-actor assertions for `GET /admin/ai-analytics` and `POST /admin/ai-analytics/book/{id}`; tenant-isolation proof for `stageCounts`/`recentBoxes` (seed two tenants, assert cross-tenant absence) and for the `$campaigns` filter list (tenant A's dropdown never contains tenant B's campaign names); supplying another tenant's `campaign_id` falls back to the unfiltered-but-own-tenant-scoped view with no error (§9); `markBooked` against another tenant's row returns 404 and leaves it unmutated; `markBooked` against the actor's own row succeeds and redirects with the existing success message (legitimate-actor path preserved).
3. **`tests/Feature/Security/AiSettingsSecurityTest.php`** — a guest, and an authenticated user lacking `manage ai_settings` (including an ordinary customer account, proving customers do **not** gain access merely because the other two surfaces are customer-facing, §3.4/§6), both receive redirect-to-login or 403 respectively for `GET`/`POST /admin/ai-brain`; an authenticated admin holding `manage ai_settings` receives 200 on `GET` and can successfully update `system_prompt`/`model` via `POST`, with the `ai_settings` table row reflecting the new values (legitimate-actor path preserved); posting with a missing `system_prompt` or `model` field is rejected by validation with no mutation to the existing row (§11, §12).
...
The exact complete-suite pass count must be reported at implementation time, never estimated or copied from any prior contract's own baseline — matching the same discipline the merged Slice-3 contract's own §8 established. **This contract itself does not run either command** — it is a docs-only contract-drafting pass; both are reported here as required future commands only.
...
4. `grep -n "->middleware(" routes/web.php` → must show the 6 new middleware chains on exactly the 6 lines named in §13 item 1, and must **not** show one on the `admin.ai_variants.update` line (§3.5 row 5, §17).
