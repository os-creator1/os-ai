...
2. Resolve the acting Customer as the authenticated user's own `Customer` record (`User::customer()`, already an existing relationship) — never a different or caller-supplied customer. `Business.customer_id` is always the actor's own tenant record for this slice; there is no on-behalf-of or cross-customer creation.
3. The `WorkspaceController` creation action reuses the existing `App\Http\Requests\Business\UpsertBusinessIdentityRequest` directly — it is already a generic Business request (not Workspace-specific or onboarding-only) that owns the complete Business identity contract: validation rules, `BusinessIndustry` enum validation, `industry_other` handling, `country_code`/`currency_code` normalization, and blank nullable-field normalization. `UpsertBusinessIdentityRequest` is not modified by this slice. No new Form Request is created; the controller only type-hints it and consumes its validated payload before calling the manager.
4. Delegate the mutation entirely to `WorkspaceManager::createBusinessInWorkspace()`. Workspace and customer authority remain enforced exclusively by that existing domain method (owner-or-active-Admin authority, active-Workspace requirement) — never reimplemented in the controller. No alternate validation or authorization algorithm is introduced anywhere in this slice.
