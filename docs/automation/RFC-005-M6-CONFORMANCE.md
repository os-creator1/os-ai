...
**RELEASE-READINESS PASS — evidence-based conformance audit complete.** This document is an evidence-based conformance audit of RFC-005 (Business Usage Billing and Wallets) as actually implemented in this repository, covering Milestones 1–5, Amendment 1 (Usage Meter Identity), and all nine post-M5 remediations. Every row below cites concrete file, method, migration, event, exception, or test evidence found by direct inspection during this pass — no row is marked PASS merely because an expected file exists, a prior document claimed it, or it "sounds correct."
...
- Prior milestone/remediation governance evidence relied on: the M1–M5 contract/implementation PR chain, the Amendment 1 slice sequence, and all nine post-M5 remediation contract/implementation/closure documents (full PR/SHA table reproduced below) — cross-checked against actual repository state rather than trusted at face value, per the contract's own "re-derive from scratch" discipline.
...
Test evidence: `TopUpStateMachineTest.php` (18 methods) — `::test_manual_top_up_creates_a_checkout_session_not_a_payment_intent`, `::test_manual_top_up_succeeds_without_a_pre_saved_default_instrument`, `::test_checkout_session_completed_webhook_confirms_a_manual_top_up`, `::test_a_payment_intent_event_cannot_confirm_a_manual_top_up_attempt`; `FundingAttemptPayerConsentTest.php::test_auto_recharge_still_creates_an_off_session_payment_intent_and_snapshots_the_instrument_at_creation`.

### Webhook verification, idempotency, claim/lease/exhaustion, disposition, retention/reconciliation — including the reconciliation race fix, fair round-robin retry-reclaim, `MAX_ATTEMPTS_CEILING`, dual-reference/metadata-hint resolution (§21, M3, R5, R8)

**PASS.** `PaymentProviderEventRetryPolicy::MAX_ATTEMPTS_CEILING = 20` (`app/Library/Usage/PaymentProviderEventRetryPolicy.php:22`), with `resolvedMaxAttempts()` clamping the configured value via `max(0, min(config(...), self::MAX_ATTEMPTS_CEILING))` — confirmed present, hard-ceilinged regardless of configuration. `ProcessPaymentProviderEvent`'s claim/lease query and `RetryStuckPaymentProviderEvents` implement the corrected fair round-robin reclaim (R5/R8). Provider-object resolution uses the event's own `metadata` hint, never `event_type` alone, resolving against exactly one local table per hint.

Test evidence: `WebhookClaimExhaustionTest.php` (bounded reclaim regression), `WebhookTerminalOutcomeTest.php`, `WebhookEventDispositionTest.php`, `WebhookExhaustedPayloadPurgeTest.php`, `ProviderIdentityUniquenessTest.php`, `ProviderObjectResolutionTest.php`, `WebhookMetadataMismatchTest.php` — all confirmed present under `tests/Feature/Usage/`.
...
Test evidence: confirmed present and passing in the Usage regression run recorded below (`919 passed`), including the funding-confirmation concurrency scenario coverage carried forward from R6's own implementation PR (#146).

### Receipt issuance — first real `business_billing_receipts` implementation, exact five financial-success dispatch points (§23, R3)

**PASS.** `database/migrations/2026_08_27_120001_create_business_billing_receipts_table.php` — the first real implementation of the receipt table M3 had deferred. `UsageWalletManager::ensureFundingReceipt()`/`findFundingReceipt()` (via `BusinessBillingReceiptRepository`) and `UsageBillingCheckoutManager::ensureFundingReceipt()` confirmed present and wired at the financial-success dispatch points.
...
**PASS.** `app/Http/Controllers/Admin/UsageBillingController.php` and `AdditionalBusinessSlotAgreementController.php` confirmed to require an explicit `reason` on administrator retry actions: `retryFundingAttemptAsAdministrator($attemptModel, (int) Auth::id(), (string) $request->validated('reason'))` and `retrySlotRenewalAsAdministrator($chargeModel, (int) Auth::id(), (string) $request->validated('reason'))`. `app/Library/Usage/UsageBillingPresenter.php`, `UsageBillingCheckoutManager.php`, and `UsageWalletManager.php` confirmed to use `bcmath` string arithmetic for aggregate computation (`bcadd`/`bcsub`/`bcmul`), never float arithmetic on money. Exhausted-events admin route confirmed: `routes/admin.php:722` — `Route::post('provider-events/{event}/dispose', 'PaymentProviderEventController@dispose')`.
...
All six locked commands (`RFC-005-M6-CONTRACT.md` §6) were run against the disposable `ultimatesms_testing` database, using the canonical local test-only `APP_NAME="AI Business OS"` environment value (kept local/uncommitted per the governing instruction) so the environment-dependent Branding test runs against its intended configuration.

| # | Command | Result |
|---|---|---|
| 1 | `php artisan test tests/Unit/Usage tests/Feature/Usage` | **PASS — 919 passed (4129 assertions)**, duration 153.63s |
| 2 | `php artisan test tests/Unit/Entitlement tests/Feature/Entitlement` | **PASS — 317 passed (851 assertions)**, duration 62.26s |
| 3 | `php artisan test tests/Unit/Workspace tests/Feature/Workspace` | **PASS — 779 passed (2038 assertions)**, duration 115.29s |
| 4 | `php artisan test tests/Unit/Business tests/Feature/Business` | **PASS — 251 passed (645 assertions)**, duration 38.43s |
| 5 | `php artisan test tests/Unit/Opportunity tests/Feature/Opportunity` | **PASS — 1117 passed (3966 assertions)**, duration 132.78s |
| 6 | `php artisan test --stop-on-failure` (full suite) | **PASS — see exact recorded output below** |

Every command discovered a positive test count and finished with zero failures — this satisfies §37's "full aggregate §35 test set passing" requirement and the contract's own "reject zero-test success" discipline.

**Command 6 (full suite) exact output**, recorded verbatim, no invented count:

```
Tests:    3514 passed (12255 assertions)
Duration: 469.18s
[exited with code 0]
```

The last visible test immediately before that summary was `Tests\Feature\Workspace\WorkspaceTransitionsMigrationSchemaTest` › `migration up down up cycle is clean on an isolated database`. This exact count (3514) matches the contract's own mechanically reconciled expected value (§ "The 3517-versus-3514 discrepancy, mechanically resolved this pass" in `RFC-005-M6-CONTRACT.md`) precisely — the 3517 static `grep -c "public function test_"` count, minus 57 methods excluded by `phpunit.xml`'s `<groups><exclude>` block, plus 1 (`ExampleTest::testBasicTest()`, a non-underscore-prefixed method grep cannot match) — with ordinary `#[DataProvider]` fan-out accounting for the rest, exactly as the contract itself derives.
...
`git diff --check`: **GIT_DIFF_CHECK_PLACEHOLDER**
