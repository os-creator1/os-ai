...
**E-1's exclusion, conservatively.** §8.2 excludes changes that an Attention
item or an Opportunity "touching those metrics" explains, and no canonical
source relates an Attention or Opportunity type to a metric. Rather than
invent that relation, E-1 treats **any** raised Attention type, or **any**
Opportunity first detected in the period, as a possible explanation. It can
stay silent where AI might have helped; it can never pay for an explanation a
rule already gives. Relating types to metrics is owner direction for a later
amendment.
