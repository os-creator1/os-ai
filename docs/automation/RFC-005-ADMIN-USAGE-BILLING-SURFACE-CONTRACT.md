...
Locked design points:
- **`retail_revenue_micro = SUM(gross_amount_micro)`, no `ROUND()` needed or applied.** `gross_amount_micro` is itself an integer column (`unsignedBigInteger`) — summing integers is exact, with no fractional component ever introduced, so wrapping it in `ROUND()` (as Round 1 did) was harmless but unnecessary; Round 2 removes it for clarity.
- **`provider_cost_micro = ROUND(SUM(CAST(provider_cost_micro AS DECIMAL(20,0)) * quantity), 0)` — corrected this round from `DECIMAL(20,6)` to `DECIMAL(20,0)`.** `provider_cost_micro` is schema-valid up to 20 integer digits (`unsignedBigInteger`); `DECIMAL(20,0)` reserves all 20 digits for the integer part, matching the column's own genuinely integer, no-fractional-part nature exactly — `DECIMAL(20,6)` reserved 6 of those 20 digits for a fractional part this column never has, leaving only 14 integer digits and risking overflow/truncation for large, schema-legal values. The multiplication against `quantity` (`decimal(14,6)`) remains exact `DECIMAL` arithmetic performed by MySQL, never an implicit float — MySQL's own decimal-multiplication rule (`DECIMAL(M1,D1) × DECIMAL(M2,D2) → DECIMAL(M1+M2, D1+D2)`, capped at MySQL's own 65-digit maximum) yields `DECIMAL(34,6)` here (`20+14=34` total digits, `0+6=6` decimal digits), comfortably within that cap — doing this computation in SQL removes the need for `bcmath` entirely, and no `bcmath` call is added to the controller.
- **`margin_micro` is no longer its own, independently-computed SQL expression.** It is computed once per feature-key row, in PHP, immediately after the query returns, as a plain integer subtraction of the two already-materialized, already-rounded aggregate values (`$row->retail_revenue_micro - $row->provider_cost_micro`) — this is a light, per-row finalization on an already-small, already-aggregated result set (at most one row per feature key), not a return to loading raw ledger rows; the aggregation itself still happens entirely in SQL. **This guarantees the invariant `margin_micro === retail_revenue_micro − provider_cost_micro` holds for every returned row, always, by construction** — there is no longer a second, independently-rounded expression that could diverge from it at a rounding boundary.
- **Rounding rule, locked: SQL `ROUND(..., 0)` to the nearest whole micro (MySQL's own default round-half-away-from-zero for `DECIMAL` values), applied only to `provider_cost_micro`** — `retail_revenue_micro` needs no rounding (already an exact integer sum) and `margin_micro` is a plain integer subtraction of two already-rounded integers, not an independently-rounded value.
...
28. `test_the_margin_aggregate_sums_multiple_usage_charge_and_usage_overage_charge_rows_correctly` — **new this round.** Seeds two ledger rows for the same Business, period, and `feature_key` — one `usage_charge`, one `usage_overage_charge` — each with its own `gross_amount_micro`/`provider_cost_micro`/`quantity`. Calls `marginAggregateForBusiness()` directly. Asserts `retail_revenue_micro`, `provider_cost_micro`, and `margin_micro` each equal the exact, hand-computed sum across both rows.
29. `test_the_margin_aggregate_rounds_a_half_micro_boundary_consistently_between_cost_and_margin` — **new this round — the direct proof of Correction Round 2's own item 3 fix.** Seeds two rows for the same feature key: row A (`provider_cost_micro = 1`, `quantity = 0.5`, `gross_amount_micro = 10`), row B (`provider_cost_micro = 2`, `quantity = 0.5`, `gross_amount_micro = 10`) — the combined cost contribution sums to exactly `1.5`, a genuine half-micro boundary. Asserts `provider_cost_micro` rounds to `2` (MySQL's own round-half-away-from-zero), `retail_revenue_micro` is `20`, and `margin_micro` is exactly `18`. This is the case that would have returned `19` under the pre-Round-2 design (`ROUND(20 − 1.5, 0) = 19`, one micro more than `retail_revenue_micro(20) − provider_cost_micro(2) = 18`) — this test would have failed against that design and passes against the corrected one.
30. `test_the_margin_aggregate_does_not_overflow_for_a_provider_cost_micro_value_exceeding_fourteen_integer_digits` — **new this round — the direct proof of Correction Round 2's own item 3 fix.** Seeds one `usage_charge` row with `provider_cost_micro = 99999999999999999` (17 integer digits — schema-valid for an `unsignedBigInteger`, and exceeding `DECIMAL(20,6)`'s own 14-digit integer capacity) and `quantity = 1`. Asserts the returned `provider_cost_micro` aggregate equals that exact value, unmodified — proving `DECIMAL(20,0)` carries it through with no overflow or truncation.
31. `test_the_margin_aggregate_is_isolated_by_business_and_period` — **new this round.** Seeds ledger rows for the target Business/period, a different period for the same Business, and the same period for a different Business. Asserts only the target Business/period's own rows are reflected in the returned aggregate.
32. `test_the_margin_aggregate_always_satisfies_margin_equals_revenue_minus_cost` — **new this round — the general invariant proof Correction Round 2's own item 3 requires.** Seeds several rows across multiple feature keys with varied `gross_amount_micro`/`provider_cost_micro`/`quantity` combinations, including at least one that rounds unevenly. Asserts, for every row `marginAggregateForBusiness()` returns, `margin_micro === retail_revenue_micro - provider_cost_micro` exactly.
...
| 13 | `app/Repositories/Contracts/BusinessUsageLedgerEntryRepository.php` | REQUIRED (modified) | Three new methods: `forBusinessPaginated()`, `findByCorrelationKey()`, `marginAggregateForBusiness()` — the last corrected in Correction Round 2 for exact `DECIMAL(20,0)` precision and internally-consistent rounding (§2.4, §2.5). |
