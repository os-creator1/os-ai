...
Confirmed by direct repository search: no `docs/automation/RFC-005-M6-CONTRACT.md` or `docs/automation/RFC-005-M6-CONFORMANCE.md` file, and no `chore/rfc-005-m6-contract`/`agent/rfc-005-m6` branch existed anywhere in this repository before this contract. **The precise claim is narrower than "no pull request ever mentions M6":** before this contract, no RFC-005 M6-specific contract, release-readiness/conformance branch, or M6 implementation/release PR existed. Prior pull requests already refer to M6 as forbidden/future scope — for example RFC-005 M4's own contract PR #103 and implementation PR #105 — exactly as every M1–M5 contract's own forbidden-scope list does. Every such existing mention of "M6" across `RFC-005-M1-CONTRACT.md` through `RFC-005-M5-CONTRACT.md` and their correction documents consistently and exclusively refers forward to this same conformance/deployment/tag milestone — never a feature scope, never an M6-specific governance object of its own, and never invented by this contract.

### No RFC-005 tag exists yet

`git ls-remote --tags origin` confirms exactly three pushed annotated tags on `origin`: `rfc-002-opportunity-engine`, `rfc-003-workspace-and-business-account-core`, and `rfc-004-plans-and-business-feature-entitlements`. **`rfc-001-business-core` is not present on `origin`** — it exists only as a local annotated tag object in this clone (`git cat-file -t rfc-001-business-core` reports `tag`), the same historical local-vs-origin inconsistency `RFC-004-M4-CONTRACT.md` already recorded and did not repair. This contract does not fabricate remote presence for `rfc-001-business-core` and does not push or repair it — that remains entirely outside this contract's scope. No `rfc-005-*` tag exists on `origin` or locally.
...
- RFC-005 §39 also lists further open product/commercial decisions — exact retail rates (item 1), default Business monthly spend cap (item 2), default per-feature limits (item 3), the auto-recharge default threshold (item 4), the owner/operator Agency subsidy policy (item 5), Agency client rebilling timing (item 7), the exact v1 add-on roster and pricing (item 8), per-feature platform safety-limit ceilings (item 9), and currency/multi-currency scope (item 10), plus the exact default monthly auto-recharge cap (item 12) — none resolved by M1–M5, none resolved by this contract, and none required to be resolved by RFC-005 §37's own acceptance-criteria text.
- **§39 item 13 — additional-slot agreement `payment_lapsed` grandfathered-capacity revocation policy — re-confirmed still open, not omitted.** Directly re-read from `RFC-005-M4-CONTRACT.md`'s own open-decision table (item 13): *"Blocks no structural M4 implementation; explicit permanent non-goal until separately authorized"* — M4 implements every RFC-defined lapse/recovery mechanism (`payment_lapsed`, `payment_lapsed_at`, stopped automatic renewals, manual/owner recovery, `payment_lapsed_cleared_at`, forward-only `next_renewal_at` recomputation) but performs **zero** Business-slot revocation of any kind. A future revocation policy requires its own explicit, separate human decision/authorization. This is not a hidden technical or tag blocker — M4's own contract already classified it as blocking no structural implementation, and no section of RFC-005 §36–§38 names it as an M6 or tag precondition — but it remains, like the other §39 items above, an open production-launch-adjacent product-policy question distinct from the tax/VAT legal/compliance gate (item 6). §4 below requires the future conformance document to preserve this exact distinction rather than silently omit it or invent a policy.
...
**Additional-slot grandfathered-capacity revocation posture (§39 item 13).** The conformance document must record, as a repository-backed distinction and nothing more: M4 implements every RFC-defined `payment_lapsed` lapse/recovery mechanism in full, and this mechanism is audited for technical conformance like any other M4 behavior; the currently implemented revocation posture is **zero** automatic Business-slot revocation of any kind; and the grandfathered-capacity revocation policy itself remains a still-open, separate human product-policy decision, exactly as `RFC-005-M4-CONTRACT.md`'s own open-decision table already recorded (*"blocks no structural M4 implementation; explicit permanent non-goal until separately authorized"*). **M6 must not invent a revocation policy and must not add revocation code of any kind.** This item does not block RFC-005's technical conformance or its tag — no section of RFC-005 §36–§38 names it as a precondition of either — but it remains an open production-launch-adjacent product-policy question, distinct from the tax/VAT legal/compliance gate (§39 item 6).
...
Tag creation requires an explicit final human authorization **after** every gate in §6, §7, and §8 has passed. Exact tag name, confirmed directly against RFC-005 §36 item 6 and §38: `rfc-005-business-usage-billing-and-wallets`. It must be an **annotated** tag — matching the verified precedent of `rfc-002-opportunity-engine`, `rfc-003-workspace-and-business-account-core`, and `rfc-004-plans-and-business-feature-entitlements`, all three confirmed present on `origin` as real annotated tag objects via `git ls-remote --tags origin` before writing this contract. `rfc-001-business-core` is also a real annotated tag object, but only locally in this clone (`git cat-file -t` reports `tag`) — it is not present on `origin`, the same historical local-vs-origin gap `RFC-004-M4-CONTRACT.md` already recorded. This contract does not repair or push `rfc-001-business-core`; RFC-005's own eventual tag must be explicitly verified against `origin` itself (§9's own verification list below), never assumed present by analogy to a local-only tag.
