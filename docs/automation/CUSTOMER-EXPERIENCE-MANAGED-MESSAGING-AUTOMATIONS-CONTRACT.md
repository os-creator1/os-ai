...
| **C-3** | `docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS.md` | Add: $5 minimum top-up; the four auto-recharge presets; the Workspace aggregate cap; the emergency kill switch; the payer no-op rule; telecom meters as first-class; and the §11.5 **measurement-versus-wallet-debit** distinction, so a metered event is never assumed to imply a debit. **Added, Slice 3 Correction Round 1 (2026-09-09):** document `UsageWalletManager::recordMeasurement()` and the additive, RFC-005-owned `business_usage_measurements` table (`docs/automation/CUSTOMER-EXPERIENCE-SLICE-3-MESSAGING-PROVIDER-FOUNDATION.md` §4.8) as the concrete measurement-without-a-rate primitive the §11.5 distinction above requires — not made in this branch because Lane A may currently be touching this RFC document concurrently. | No |
