...
**Corrected 2026-09-08 (Correction Round 2) — this section described a pre-research state that no longer holds; the provider-mechanism confirmation it required is now recorded.** At the time this contract was first drafted, the repository contained no evidence of any Telnyx provisioning, sub-account, or managed-account capability (E-22); `SendingServer::TYPE_TELNYX` was, and remains, only a credential holder for a customer-supplied key on the legacy/BYO path. That evidence gap is what §28.3 existed to close.

`docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md` has since closed it, citing official Telnyx sources (its §3) for exactly what Telnyx permits: Messaging Profile assignment, per-profile webhook configuration, 10DLC brand/campaign registration on an ISV's end-customers' behalf, and the account/billing shape of both a shared platform account and Telnyx Managed Accounts. §28.3 records the selected launch mechanism (one shared, platform-owned, Pay-as-you-go Telnyx account, §28.3) on that evidence.

Therefore, as of this correction:

* **Verified provider capability: recorded in §28.3 and the decision document**, not absent. §10 and §11.2–§11.4 below are no longer *proposed*, contingent application behaviour — they are the specified requirements for Slice 3, which may implement them now.
* **What remains genuinely unimplemented is the code, not the confirmation.** No implementation slice may claim §11.2's isolation properties as *already true of the repository* until Slice 3 actually builds them (decision doc §7.1) — but no slice is blocked from starting that work by any outstanding provider-mechanism question. The distinction is implementation status, not architectural authorization.
* **Outbound Telnyx calls remain inert regardless of this correction** until real platform credentials and explicit runtime configuration are deliberately supplied; missing or invalid configuration must fail closed, never silently no-op as if it succeeded. Selecting a mechanism in §28.3 does not, by itself, authorize number purchasing, customer provisioning, retail charging, or production traffic — those remain gated exactly as §21, §28.1, and §28.4 already state.

### 11.2 Isolation requirements (whatever mechanism is chosen)

**Corrected 2026-09-08 (Correction Round 1).** The original single "Blast radius" row stated a guarantee that the launch-selected mechanism cannot actually provide at the shared-provider-account level (`docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md` §7, §12). Restated honestly as two distinct levels rather than one unqualified rule, so this contract never claims an isolation property the chosen launch architecture does not have:
...
| **Blast radius — application/resource level (mandatory at launch and every slice thereafter)** | A compromise, configuration error, number suspension, Messaging Profile issue, 10DLC campaign suspension, or Business-level pause affecting one Business must **never** expose, reroute, charge, or deliberately disable another Business **through AI Business OS itself**. Enforced by: one Messaging Profile per Business; one authoritative number-to-Business mapping (`BusinessMessagingIdentity`); Business-scoped wallet and usage records; a Business-scoped kill switch; fail-closed webhook routing keyed to that mapping; no customer-accessible platform credentials; per-Business rate/volume controls; fail-closed conflict detection on every inbound event. |
| **Blast radius — platform-provider failure domain (explicitly accepted for launch, §28.3)** | Under the launch mechanism (§28.3), a suspension, KYC issue, balance failure, credential compromise, or Telnyx enforcement action against the **shared platform Telnyx account** can affect every managed Business simultaneously. This is a known, disclosed, owner-accepted launch-stage risk — it is mitigated operationally (platform-only credentials; least-privilege API keys where Telnyx's permission-group model supports them; key rotation; signed-webhook verification; strict AUP enforcement; per-Business anomaly detection; Business-level sending limits; a rapid platform-wide kill switch; Telnyx balance monitoring; compliance review before activation; immutable attribution/audit records) but is **not eliminated** by any application-level control, and must never be represented as eliminated. §28.3 records the documented migration path (Telnyx Managed Accounts) that removes this specific risk once the owner accepts its commercial cost. |
...
| **Auto-recharge presets $5/$10/$25/$50, no custom amount** | — | **New (E-17).** `ConfigureAutoRechargeRequest` validates `Rule::in(presets)`; `UsageWalletManager::configureAutoRecharge()` refuses anything else. **Owner-approved (Slice 5 Correction Round 1, §28.9 resolved):** the four presets are the whole surface, custom entry stays disabled, $5 is only a visual suggestion on the page (never persisted, never consent), and automatic top-up is **off by default** until the payer explicitly enables it. |
| Monthly auto-recharge ceilings — **required, bounded, enforced in execution** | `monthly_recharge_cap_micro`, `workspace_usage_controls.monthly_aggregate_recharge_cap_micro` | **Owner-approved (Slice 5 Correction Round 1):** a Business ceiling is required to enable automatic top-up and is at most **$500/month**; the Agency Workspace aggregate ceiling across agency-paid Businesses is at most **$500/month** and is required before an agency-paid Business can be charged automatically; both are safety maxima, never defaults; the real `EvaluateBusinessAutoRecharge` → `UsageBillingCheckoutManager::initiateCharge()` path enforces them under the wallet and Workspace row locks before any provider call. **At most two automatically initiated top-ups per Business in any rolling 24 hours** (precise timestamps; manual top-ups excluded; corrected in Correction Round 2: every initiated attempt holds its slot for the whole window whatever its outcome — failure or cancellation releases only the monetary headroom, never the slot). Single policy source: `UsageWalletManager::AUTO_RECHARGE_*` / `*_MONTHLY_AUTO_RECHARGE_MAXIMUM_MICRO` constants. |
...
| **3** | **Managed messaging / provider foundation** — provider abstraction, per-Business isolation, credential custody, telecom **measurement** meters, move BYO to Settings → Advanced | 1B | **Corrected 2026-09-08 (Correction Round 2): §28.3 is resolved (`docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md`) — this slice is not blocked, and is not restricted to design-only or fake-backed work.** Slice 3 may implement the real Telnyx-backed provider adapter and the required application-level isolation (§21.2) against the selected mechanism now. Outbound Telnyx calls remain inert absent real, deliberately supplied credentials/configuration; missing or invalid configuration fails closed. **Retail rate activation is separately gated by §28.1** and is excluded from this slice. | 5 |
| **4** | **Business-phone guided onboarding** — §10 flow, compliance intake, activation state machine (§7.7), number lifecycle (§13) | 3, 5 | **BLOCKING, updated (Correction Round 2): §28.3 (mechanism) is now resolved** (`docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md`); **§28.4 (launch countries and compliance scope), §28.1 (rates), and Slice 5 (funding behaviour) remain outstanding and must all be resolved before provisioning any number** | — |
| **5** | **Wallet / payer / balance UX** — $5 minimum, auto-recharge presets, plain-language spend caps, Workspace aggregate cap, kill switch, payer visibility rules, **payer no-op fix (E-12)**, raw feature keys removed (E-14) | 1B | **Implemented; §28.9 resolved by owner approval in Correction Round 1:** fixed presets only (no custom amount), off by default with a visual $5 suggestion, $500 monthly automatic-top-up maxima per Business and per Agency Workspace, at most two automatic top-ups per Business per rolling 24 hours, the Agency payer control at Client accounts → [Business] → Billing responsibility, and payer-side financial-control authority. Funding mechanics remain rate-independent (§28.1a still gates only the retail rate card). | 3 |
...
| **3** | 1B complete (§28.3 is already recorded, Correction Round 2 — this slice may start its real implementation immediately, not only fake-backed design work) | The abstraction carries a real recorded mechanism; per-Business isolation is **implemented and verified**, not merely designed — no customer role can read a credential; BYO is relocated; measurement meters exist with **no retail rate activated**; T-PROV-1..2, T-BYO-1..2 pass. Tests may use a deterministic fake for coverage of edge cases and failure modes (a testing choice, §21.2), but passing those tests requires the real isolation behavior to exist, not a substitute for it. |
| **4** | 3 and 5 complete **and** §28.4 and §28.1 recorded (§28.3 is already recorded, Correction Round 2) | A number is acquired only after funds are reserved; renewal, grace and release behave per §13; T-PHONE-1..4 pass |
...
### 21.2 Provider-adapter scope and what must remain provider-neutral

**Corrected 2026-09-08 (Correction Round 2) — §28.3 is resolved (see §11.1, §28.3); this section's original framing ("before a blocking gate clears") described a state that no longer applies to Slice 3.** It is retained, corrected, for two things that remain true regardless: what "real" provider-adapter work means precisely, and the provider-neutrality discipline the schema and interface must carry forever, not just until a gate cleared.

**Slice 3 is authorized to build, now, against the selected mechanism (§28.3):**

* production-shaped Telnyx client code implementing the provider-agnostic interface (send, number search, number order, registration submit, status callback) expressed in **platform** vocabulary;
* encrypted credential handling for the platform's shared Telnyx credential (§11.2, §11.3);
* provider-neutral identity resolution (`BusinessMessagingIdentity`), Messaging Profile isolation, and number mapping, per-Business;
* fail-closed webhook routing and provider error mapping;
* a deterministic in-memory fake implementing the same interface, in the house pattern already used by `FakeGoogleBusinessProfileReadClient` — used for fast, deterministic test coverage, a testing choice, not a substitute for the real adapter existing;
* the per-Business isolation and credential-custody rules asserted as tests against both the real adapter and the fake;
* the measurement meters and the BYO relocation.

**What Slice 3 may not do:** invent or commit any real credential; make a live Telnyx API request as part of this correction or as part of authoring the contract/decision documents; or infer provider-side success from local persistence alone (a database row saying "sent" is never itself proof a message left the platform — the adapter must derive that from the provider's own response/webhook). Absent real credentials or configuration, every outbound call path must be safely disabled — a clear, logged no-op or failure, never a silent success and never a crash. Actual provisioning, customer-facing sending, and production traffic remain subject to Slice 4's own gates (§28.4, §28.1, Slice 5) regardless of how complete Slice 3's adapter is.

**Provider-neutrality remains a permanent discipline, not a until-the-gate-clears one:** no interface shape, column, enum value, migration, or test may encode an *assumed* Telnyx account structure that the actual, now-recorded mechanism does not require — concretely, no Managed-Account-specific identifier column (dormant, nullable, or otherwise) belongs in the launch schema (`docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md` §8), since Candidate B never uses one. If a future slice implements a different provider mode, it adds what it needs through its own additive migration at that time (decision doc §5.2, §8) — this contract does not pre-build for a mechanism it has not yet authorized.
...
| **5** | `app/Http/Requests/Customer/Business/**`, `app/Library/Usage/{BillingProfileManager,UsageWalletManager}.php`, `app/Http/Controllers/Customer/Business/UsageBilling*.php`, `app/Models/{BusinessUsageWallet,BusinessPayerAssignment}.php`, `app/Notifications/**`, `app/Console/Commands/**` (threshold alerts), `resources/views/customer/business/usage-billing/**`, `resources/lang/en/locale.php`, `database/migrations/**`; **Correction Round 1 (narrow, each mechanically required):** `app/Jobs/Usage/EvaluateBusinessAutoRecharge.php` (the real automatic-recharge execution must consult the ceilings and the rolling window), `app/Library/Usage/UsageBillingCheckoutManager.php` (**`initiateCharge()`'s AutoRecharge branch only** — the admission and durable claim under the wallet lock), `app/Repositories/Contracts/BusinessFundingAttemptRepository.php` and `app/Repositories/Eloquent/EloquentBusinessFundingAttemptRepository.php` (exactly two additive read methods that count pending automatic-top-up capacity and rolling-window attempts from the authoritative funding attempts), `app/Http/Controllers/Customer/Workspace/WorkspaceController.php` (`show()` billing-responsibility view data only) and `resources/views/customer/workspaces/show.blade.php` (the Client accounts → [Business] → Billing responsibility control only). `routes/customer.php` was **not** needed: the account-frame form posts to the existing `…/usage-billing/payer` route. | `tests/Feature/Usage/**`, `tests/Unit/Usage/**`; **Correction Round 1:** no Workspace/Security test path was needed (the account-frame tests live under `tests/Feature/Usage/Slice5/`) | `docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS.md` (§27 C-3), the slice's own contract; **Correction Round 1:** this document (§12.2, §21, §22.1, §24.1, §28.9) |
...
**Preimplementation executability correction to the Slice 3 row — Round 0
(2026-09-09).** Mechanical audit for
`docs/automation/CUSTOMER-EXPERIENCE-SLICE-3-MESSAGING-PROVIDER-FOUNDATION.md`
§2 (18-item repository trace, file:line evidence) proved that the Slice 3 row
as originally written could not deliver two of its own already-locked exit
criteria (§21.1: "per-Business isolation is **implemented and verified**, not
merely designed" and "replacing fail-open DLR attribution"): the row named no
path for the real production inbound-webhook entry point or the real
production outbound-dispatch entry points that every existing Business send
and every inbound Telnyx message actually passes through today.

**Preimplementation executability correction to the Slice 3 row — Round 1
(2026-09-09), superseding Round 0's five-path addition above with the wider,
more precise set now in the row.** A second mechanical pass (that document's
§2 item 19, plus re-examination of items 4, 7, 10-12, 16) found that Round
0's design itself could not deliver what it claimed, independent of the
allowlist question: its usage table mixed six unrelated responsibilities
under one name and called it "outside RFC-005" without changing what the
table actually was; its `business_messaging_identities.phone_number` column
could not honestly represent the already-locked "dedicated phone number(s)"
(plural) architecture; its inbound resolver used the Messaging Profile ID as
the *sole* attribution key despite the architecture's own requirement for an
authoritative number-to-Business mapping; its BYO-relocation design gated
behaviour on `manage_advanced_provider`, a permission that does not exist
anywhere in application code; and it left the fail-open
default-to-user-1 legacy webhook behaviour fully intact beside a new safe
route, which does not satisfy "replacing fail-open DLR attribution" for the
production surface that real BYO traffic actually uses. The corrected row
above separates operational transport state (`business_messaging_operations`),
RFC-005-owned usage measurement (one additive `UsageWalletManager` method
plus one additive, generic table), and bounded security auditing
(`messaging_webhook_rejections`) into three distinct stores; replaces the
single `phone_number` column with a genuine one-to-many
`business_messaging_numbers` mapping; requires two independent, cross-checked
signals for inbound attribution; gives `manage_advanced_provider` a real,
evidenced representation; and removes — rather than merely supplements — the
shared fail-open write in the existing `inboundDLR()` method, with narrow,
evidence-specific edits to `inboundTwilio()` and `inboundTelnyx()` for their
respective BYO postures. Full evidence, the corresponding executability
table, and the exact scope of every change are recorded in that document's
§2–§4.11.

**Preimplementation executability correction to the Slice 3 row — Round 2
(2026-09-09), correcting Round 1's uniqueness-enforcement mechanism and
naming the RFC-005 measurement seam's ownership at every layer.** Round 1's
three database invariants (one active-or-pending managed identity per
Business; a phone number cannot belong to two active-or-pending identities;
at most one active primary number per identity) were each specified as a
"partial unique index" — a PostgreSQL construct this repository's actual
database, MySQL 8.0 (confirmed by this repository's own CI service
containers and a real recorded MySQL 8.4 test run, that document's §2 item
20), does not support at all. The corrected design replaces every such claim
with a mechanism already merged and tested in this exact repository — a
nullable `STORED` generated guard column plus an ordinary `UNIQUE` index,
identical to `database/migrations/2026_08_16_140001_create_payment_provider_customers_table.php`'s
already-proven pattern — making the database itself, not application
validation or a row lock, the sole enforcement mechanism for all three
invariants, with archival/replacement/reactivation/concurrency behaviour and
forward/rollback/replay migration tests specified exactly. The same pass
also names, for the first time at every layer, the exact repository contract
and implementation (`BusinessUsageMeasurementRepository`/`EloquentBusinessUsageMeasurementRepository`,
both new) that `UsageWalletManager::recordMeasurement()` must delegate to,
matching RFC-005's own already-established Manager-calls-Repository-writes-table
layering rather than having the Manager write the new table directly — the
two new repository paths are added to the row above. Full evidence, the
...
| **T-WALLET-4** | The four fixed presets configure exactly; every other amount is refused at the request and at the manager; no custom amount control exists (owner-approved: custom entry stays disabled) |
| **T-WALLET-5** | Auto-recharge is **off** until the payer explicitly enables it; no automatic charge occurs before that |
| **T-WALLET-6** | The monthly Business ceiling (required, at most $500) and the Agency Workspace aggregate monthly ceiling (required for agency-paid Businesses, at most $500) each stop further automatic recharges at their exact boundary **in the real evaluation job and checkout path, under lock, before the provider call**; at most two automatic top-ups per Business in any rolling 24 hours |
...
|---|---|---|
| T-PHONE-1..4 | §28.4 countries/compliance, §28.1a rate policy. **Corrected 2026-09-08 (Correction Round 2): §28.3 mechanism is resolved and no longer gates this test** — it is retained here only because §28.4/§28.1a still do. | 4 |
| T-FUND-1..6 | §28.1a rate policy — the itemised estimate needs real retail rates. Written against the §21.2 fake with fixture rates until then. **§28.3 mechanism no longer gates this test (Correction Round 2)** — the mechanism question is resolved; only the rate policy remains open. | 4 |
...
| T-WALLET-4 (custom-amount bounds) | **Resolved (Slice 5 Correction Round 1, owner-approved):** no custom amount ships; the four presets are the whole surface and T-WALLET-4 asserts the refusal of every other amount at both boundaries. | 5 |
| T-WALLET-6 (ceiling maxima) | **Resolved (Slice 5 Correction Round 1, owner-approved):** $500 per Business per month, $500 aggregate per Agency Workspace per month, two automatic top-ups per Business per rolling 24 hours. | 5 |
...
| **C-3** | `docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS.md` | Add: $5 minimum top-up; the four auto-recharge presets; the Workspace aggregate cap; the emergency kill switch; the payer no-op rule; telecom meters as first-class; and the §11.5 **measurement-versus-wallet-debit** distinction, so a metered event is never assumed to imply a debit. **Added, Slice 3 Correction Round 1 (2026-09-09), named exactly, Correction Round 2 (2026-09-09):** document `UsageWalletManager::recordMeasurement()`, its new `BusinessUsageMeasurementRepository` contract/`EloquentBusinessUsageMeasurementRepository` implementation pair, and the additive, RFC-005-owned `business_usage_measurements` table/`BusinessUsageMeasurement` model (`docs/automation/CUSTOMER-EXPERIENCE-SLICE-3-MESSAGING-PROVIDER-FOUNDATION.md` §4.8) as the concrete measurement-without-a-rate primitive the §11.5 distinction above requires — not made in this branch because Lane A may currently be touching this RFC document concurrently. | No |
...
| **28.3** | Whether managed Telnyx sub-accounts, a single platform account with per-Business messaging profiles, or another mechanism best matches provider terms | **Amended 2026-09-08, corrected 2026-09-08 (Correction Round 1), corrected again 2026-09-08 (Correction Round 2) — resolved for launch.** `docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md` records the evidence in full. **Selected launch mechanism: one platform-owned, Pay-as-you-go Telnyx account, with one dedicated Messaging Profile and dedicated phone number(s) per Business**, resolved via an internal `BusinessMessagingIdentity` record — never a customer-entered credential (decision doc §5–§9). **Round 1's initial selection of Telnyx Managed Accounts (with Rollup Billing) is withdrawn as the launch mechanism**, not because the research was wrong, but because it surfaced a real commercial gate this contract had not priced in: Managed Accounts require a paid Telnyx commit plan — Starter ($1,000/month minimum spend) or Growth ($2,000/month minimum spend), both quarterly-billed, or Enterprise ($5,000+/month) — **unavailable on Pay-as-you-go** — plus Telnyx's explicit manager-account approval. A mandatory $1,000+/month fixed provider commitment before corresponding customer revenue exists is commercially unacceptable for launch under this platform's locked low-cost/no-included-telecom-credit model (§10.6, §12.1). **Managed Accounts remain the documented scale-stage migration target** (decision doc §5's migration outline), evaluated by the owner — never by an automatic threshold in application code — when usage naturally approaches the commit level, revenue comfortably supports it, Telnyx offers an affordable startup/partner exception, or provider-level suspension isolation becomes worth more than the added fixed cost. | The repository provided zero evidence of Telnyx capability before this pass (§11.1); that gap remains closed by the decision document's cited evidence, which is preserved in full for both candidates. **Gates, corrected:** the launch mechanism above is **not** gated on any Telnyx commercial commitment or approval — Slice 3's real (non-fake) provider foundation may be implemented against it now, with actual Telnyx API calls remaining inert until real platform credentials/configuration are deliberately supplied (never before then; missing or invalid configuration fails closed). §21.2's provider-neutrality discipline is unaffected: **no Managed-Account-specific column of any kind — not even a dormant, nullable one — exists in the launch schema at all** (Correction Round 2; decision doc §8); a future dedicated-account mode, if ever authorized, adds its own provider-neutral column (e.g. `provider_account_reference`, never Telnyx-product-named) via its own ordinary additive migration at that time. Slice 4 remains blocked exactly as originally contracted — on §28.4 (launch countries/compliance), §28.1 (retail rates), and Slice 5 (funding) — with no additional commercial/approval precondition; the Managed-Account commit-plan and approval question is a **future migration precondition only**, not a Slice 3 or Slice 4 blocker. **Remaining prohibitions, unchanged:** no Telnyx API call, account, number, brand, campaign registration, or rate activation of any kind in this document or the decision document itself; real provisioning still requires deliberately supplied production credentials and, separately, §28.1/§28.4/Slice 5. Selecting this mechanism does not, by itself, authorize number purchasing, customer provisioning, retail charging, or production traffic. |
...
| **28.9** | **Auto-recharge bounds — RESOLVED by owner approval (Slice 5 Correction Round 1, 2026-09).** Approved: automatic top-up **off by default** until the actual payer enables it (a saved card, a manual top-up or an unrelated form never enables it; consent stays attributable via `auto_recharge_consented_at/_by_user_id`); per-recharge amounts **$5 / $10 / $25 / $50 only**, **no custom amount** (a crafted amount is rejected by the FormRequest and the manager independently); **$5 visually preselected** when disabled and no valid preset is saved (never persisted on GET, never consent, never a charge); **$500 per Business per month** and **$500 aggregate per Agency Workspace per month** as automatic-charge safety maxima (not defaults, not included credit, not manual-top-up or spending-cap limits — the payer deliberately chooses a ceiling at or below them); **at most two automatic top-ups per Business in any rolling 24 hours** (precise timestamps, manual top-ups excluded, the Agency aggregate applies independently). Single authoritative source: `UsageWalletManager` policy constants. | The former recommended $5–$500 custom range is withdrawn: custom entry does not ship. | Owns T-WALLET-4..6 (§24.1), all three now ungated. |
...
1. **A paid "extra Business slot" checkout already exists** (RFC-005 M4
   ...
   agreement is separate Billing work. (`updateCatalogPricing()` now refuses
   the ratio itself — correction round 1 below.)
...
(d) A new location needs a name and a physical service mode. (e) A
cross-Workspace reassignment re-evaluates grandfathering against the target
plan — resolved by correction round 1 below.

**Correction round 1 (2026-09-11, review of PR #251) — two merge blockers.**

1. **Extra-Business billing stays dead.** A NULL ratio alone was not enough:
   `updateCatalogPricing()` could write one back, and the hard-coded
   Core/Growth `0..2` allocation rule still accepted extra Business slots.
   Now (a) `updateCatalogPricing()` refuses a non-null
   `additional_business_slot_price_ratio` for any catalog row that offers no
   additional Business capacity (`business_slot_max <= business_slot_included`,
   read from the row, not the tier name) before any update, audit row or
   event; Agency keeps its own refusal. (b) Every path that can raise
   `workspace_plan_assignments.additional_business_slots` —
   `assignFirstPlan()`, `changePlan()`, `setAdditionalBusinessSlots()`,
   `allocateAdditionalBusinessSlotsFromVerifiedPayment()` and the operator
   backfill `WorkspaceEntitlementBackfillV1` — derives its limit from
   `business_slot_max − business_slot_included`, so none can raise it on the
   corrected Core/Growth rows. A reduction stays valid, so a counter left from
   the 3/5 catalog can be brought to zero but never raised. RFC-005 checkout,
   Stripe and payment code are unchanged; the checkout reaches allocation only
   through the guarded method. Tests that prove the generic engine pin a
   bounded 3/5 row in their own transaction
   (`tests/Feature/Entitlement/Concerns/PinsBoundedBusinessSlotCatalog.php`);
   fixtures that allocated two extra Business slots on Core now allocate none.
2. **Cross-Workspace reassignment.** `WorkspaceManager::reassignBusiness()`
   now calls `EntitlementManager::reconcileLocationCapacityForReassignment()`
   in its own transaction, right after the target's Business-slot check and
   before any mutation; the lock order stays every Workspace ascending →
   Business → its location rows. A Business holding a paid
   `additional_location_slots` allocation is refused
   (`LocationAllocationNotPortableException`) before anything changes — the
   allocation is neither cleared nor transferred. Against a bounded target the
   complimentary allowance is recalculated fresh from the current active
   locations (`max(0, active − target included)`, the same rule `changePlan()`
   and archive use); every location is kept and none is added. An unlimited
   target keeps the allowance unused. The target Workspace records a
   `capacity_grandfathered` row with payload source `business_reassignment`
   whenever the allowance changes or the Business arrives carrying one. The
