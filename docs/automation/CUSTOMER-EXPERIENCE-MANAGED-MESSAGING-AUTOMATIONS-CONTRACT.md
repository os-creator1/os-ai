...
| **28.3** | Whether managed Telnyx sub-accounts, a single platform account with per-Business messaging profiles, or another mechanism best matches provider terms | **Amended 2026-09-08 — architecturally resolved, activation-conditional.** `docs/automation/TELNYX-MANAGED-MESSAGING-ARCHITECTURE-DECISION.md` records the evidence in full. **Selected mechanism: Telnyx Managed Accounts, with Rollup Billing enabled, one Managed Account per Business** — the only Telnyx-documented mechanism that satisfies §11.2's per-Business suspension-isolation requirement at the account level, and Telnyx's own stated solution for a reseller/MSP/multi-tenant SaaS platform (decision doc §3–§5, citing `support.telnyx.com/en/articles/4951492-managed-accounts` and the Managed Accounts OpenAPI-generated reference, both accessed 2026-09-08). **Activation condition (new, not previously named in this contract):** Managed Accounts require (a) the platform owner's commercial commitment to a paid Telnyx commit plan — Starter ($1,000/month minimum spend) or Growth ($2,000/month minimum spend), both quarterly-billed, or Enterprise ($5,000+/month); Managed Accounts do not exist on Pay-as-you-go; and (b) Telnyx's explicit approval of manager-account status. Neither is recorded as satisfied by this amendment. | The repository provided zero evidence of Telnyx capability before this pass (§11.1); that gap is now closed by the decision document's cited evidence. **Gates, updated:** the mechanism itself no longer blocks Slice 3's design-only work (already true under §21.2) or Slice 3's fake-backed implementation; it continues to block **any real (non-fake) Telnyx-backed implementation** in Slice 3 and **all** of Slice 4 until the activation condition above is separately recorded as satisfied, with the date and the specific plan/approval confirmed. §21.2 continues to define exactly what may be built in the interim and continues to forbid encoding any assumed account structure beyond what the decision document's interface (its §14/§15) already documents. **Remaining prohibitions, unchanged:** no Telnyx API call, account, number, brand, campaign registration, or rate activation of any kind until both the commit-plan decision and Telnyx's approval are recorded in writing here or in an amendment to this row. |
