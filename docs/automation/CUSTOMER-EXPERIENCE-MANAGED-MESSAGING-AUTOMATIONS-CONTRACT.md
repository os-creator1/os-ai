...
| **3** | `app/Library/Messaging/**` (new), `app/Library/Messaging/Contracts/**` (new), `app/Models/BusinessMessagingIdentity.php` (new), `app/Http/Controllers/Customer/Business/MessagingChannelsController.php`, `app/Enums/Messaging/**` (new), `resources/views/customer/business/MessagingChannels/**`, `resources/views/customer/settings/advanced/**` (new), `config/services.php`, `config/messaging.php` (new), `app/Providers/AppServiceProvider.php` (binding only), `database/migrations/**`; **Preimplementation executability correction (2026-09-09, see `docs/automation/CUSTOMER-EXPERIENCE-SLICE-3-MESSAGING-PROVIDER-FOUNDATION.md` §3):** `app/Http/Controllers/Customer/DLRController.php` (existing — narrowly, one new fail-closed managed-Telnyx-webhook method only; every one of its ~60 other provider methods, and its existing `inboundTelnyx()`/`inboundDLR()` BYO behaviour, stay untouched), `routes/public.php` (existing — narrowly, one new route line for that method under the already CSRF-exempt `inbound/*` prefix; no existing route line is modified or removed), `routes/customer.php` (existing — narrowly, new route additions only for the BYO-relocated `resources/views/customer/settings/advanced/**` surface; no existing Slice-3-owned `MessagingChannelsController` route modified), `app/Repositories/Eloquent/EloquentCampaignRepository.php` (existing — narrowly, a pre-dispatch managed-identity resolution/delegation call inserted immediately before the existing provider `switch` in `quickSend()`; the switch itself and everything downstream of it, including every other provider case, stay untouched), `app/Models/Campaigns.php` (existing — narrowly, the same pre-dispatch resolution/delegation call inserted before its own internal dispatch `switch`; no other method touched) | `tests/Feature/Messaging/**` (new), `tests/Feature/Security/**`, `tests/Feature/Usage/**`; **Preimplementation executability correction:** `tests/Feature/Business/**` (narrowly, the existing `MessagingChannelsTest.php` and new production-entry-point integration tests for the two delegation points above) | the slice's own contract; **restate the superseded B2 docblock rules** (§27 C-5) |
...
**Preimplementation executability correction to the Slice 3 row (2026-09-09).**
Mechanical audit for `docs/automation/CUSTOMER-EXPERIENCE-SLICE-3-MESSAGING-PROVIDER-FOUNDATION.md`
...
path for the real production inbound-webhook entry point
(`DLRController::inboundTelnyx()`/`inboundDLR()`, reached by
`routes/public.php`'s `inbound/telnyx/{gateway?}`) or for the real production
outbound-dispatch entry points (`EloquentCampaignRepository::quickSend()` and
`Campaigns`'s own dispatch switch) that every existing Business send and every
inbound Telnyx message actually passes through today. Without amending the
row, Slice 3 could only have built new, unused `app/Library/Messaging/**`
code next to the legacy paths — proving isolation properties against a
fake/test harness while production traffic kept using the fail-open legacy
behaviour unchanged, which is exactly the outcome §3 of that document was
required to rule out. The five narrow additions above are the mechanically
minimal set that lets Slice 3 delegate from the real entry points without
rewriting or otherwise touching either god-class (`DLRController.php`'s other
~60 provider methods; `SendCampaignSMS.php`'s 18,188 lines) — full evidence,
the corresponding executability table, and the exact scope of each change are
recorded in that document's §2–§3.
