...
## 19. Retry/reclaim — index-supported branch queries, one per centrally-bounded processing-attempt bucket, fairly interleaved at two levels, entirely disabled at zero, and never silently exceeding the caller's requested limit
...
**Six linked defects have now been found and corrected across this section's revisions:**
...
5. **`max_attempts = 0` produced permanent redispatch churn instead of disabling retry.** The received-recovery branch's own query has never depended on `$maxAttempts` at all — it is bounded only by `received_at`/the grace interval. Normalizing `max_attempts` to `0` correctly disabled the `failed` and stale-processing branches (`attempts < 0` is always false), but the received branch kept executing, kept selecting the same stale rows, and kept issuing `dispatch()` calls for events whose own downstream attempt-tracking is disabled — work that can never reach a terminal, retry-tracked state under a `0`-attempts policy. **Corrected**: normalizing `max_attempts` to `0` now means *all* provider-event retry/reclaim is disabled, uniformly across every branch, not merely the two branches whose `WHERE` clause happened to already reference `$maxAttempts`.
6. **Silently raising `$limit` broke the caller's own resource bound — confirmed and corrected this pass.** The fairness floor introduced in the previous pass (`$limit = max($limit, 3, $maxAttempts)`) could turn a caller's literal request for `1` row into a return (and dispatch) of up to `20` — a parameter named `limit` must be a hard ceiling; idempotency at the business-logic layer does not authorize a repository method to silently exceed the workload a caller explicitly asked for. **Corrected**: the floor is now an explicit, upfront validation — a requested limit below the required fairness floor throws a built-in `\InvalidArgumentException` before any query executes, rather than being silently raised.
...
### Per-branch queries — disabled entirely at zero, validated (never silently raised) otherwise, then queried once per eligible, ceiling-bounded attempt bucket, each fully sargable
...
**`$maxAttempts === 0` is checked first, before any query of any kind — including the `received` branch, which was previously independent of `$maxAttempts` entirely.** `normalizedMaxAttempts()` already maps every non-positive raw configured value (zero, or negative) to `0`; `retryable()`'s own identical `max(0, min(...))` clamp confirms this defensively. When `$maxAttempts === 0`, this method returns an empty `Collection` immediately: zero database queries, zero selected candidates, zero dispatches. `ProcessPaymentProviderEvent`'s own `claim()`/processing logic is unmodified and untouched by this policy — this correction disables the *scanner's own recovery* of any state class, not the ordinary, synchronous, webhook-triggered first dispatch that happens outside this scanner entirely. **The administrator's existing exhausted/disposition surface is unaffected** — `PaymentProviderEventController`'s `exhausted()`/`dispose()` calls continue to use the identical normalized `0`, so a `failed` or expired-`processing` row with `attempts ≥ 0` (i.e., any row that was ever attempted at all) still correctly appears in, and can still be manually disposed from, the existing administrator queue; only the *automatic scanner* stops touching any row.
...
- **Per-query database-read bound:** when `$maxAttempts > 0`, every individual query — the one `received` query, the one `failed` query, and each of the (`≤ 20`) stale-processing bucket queries — is independently `LIMIT $requestedLimit`-bounded and fully served by its own declared index, with exactly one navigable range predicate. No single query ever reads more than `$requestedLimit` rows, where `$requestedLimit = max(1, min($limit, 200))` and the method has already thrown, rather than proceeded, if `$requestedLimit < max(3, $maxAttempts)`.
- **Total database-read bound for one non-throwing `retryable()` call: `(2 + $maxAttempts) × $requestedLimit` rows, with `$maxAttempts ≤ 20` and `$requestedLimit ≤ 200` always** — worst case (`$maxAttempts = 20`, scanner's real `$limit = 200`) is `22 × 200 = 4,400` rows examined at most, a fixed, mechanically provable ceiling independent of table size or configuration; `0` rows examined when `$maxAttempts = 0`.
- **Selected/dispatch bound: at most `$requestedLimit`, never more — and never silently more than the caller asked for.** `retryable()` returns fewer than `$requestedLimit` whenever eligible candidates across the `received`/`failed`/stale-processing-bucket branches, combined, are exhausted before that count is reached — a materially empty or sparse table is not an error and does not violate this bound. For the real scanner, this maximum is exactly `200`. When `$maxAttempts = 0`, the count is exactly `0`.
- **Degenerate cases, explicitly locked:**
  - **Negative or zero configured `max_attempts`:** `normalizedMaxAttempts()` returns `0`; `retryable()`'s own internal `max(0, min(...))` clamp independently confirms `0` for any non-positive input; the method returns an empty collection immediately — zero queries of any kind, zero candidates, zero dispatches. This is a deliberate, binding policy: `max_attempts = 0` means provider-event processing/retry is disabled, uniformly across every state class, not merely the two branches whose own `WHERE` clause happens to reference `$maxAttempts`.
  ...
  - **A `retryable()`-level `$limit` smaller than `max(3, $maxAttempts)` (only reachable when `$maxAttempts > 0`):** rejected outright — `\InvalidArgumentException`, zero queries, zero mutation — never silently raised and never silently truncated. A requested limit exactly equal to `max(3, $maxAttempts)` is accepted and proceeds normally.
...
| 8 | `tests/Feature/Usage/PaymentProviderEventRetryReclaimTest.php` | 38 |
...
**Total: 187 methods across 13 files.**
...
### `PaymentProviderEventRetryReclaimTest.php` (38) — proves §19; +2 methods in the first exceptional pass for index-presence and sparse/large-table behavior (Blocker 4); +9 methods in the second pass for the fair-interleave/index-ordering correction (Defects 1–2); +6 methods in the third pass for the per-attempt-bucket stale-processing correction; +4 methods in the fourth pass for the centrally-enforced ceiling and the (now-withdrawn) fairness floor; +5 methods this pass for the zero-disables-retry policy and the throw-based fairness validation, methods 29/33 renamed in place, method 31's own coverage strengthened
...
29. `test_a_non_positive_configured_max_attempts_value_disables_retry_entirely_zero_queries_zero_candidates_zero_dispatches`
...
34. `test_a_negative_raw_configured_max_attempts_value_normalizes_to_the_same_disabled_retry_behavior_as_zero`
...
9. **Duplicate delivery, a stranded `received` row, and genuine concurrency never apply the same financial, refundable-paid, or state effect twice; a stranded event is actually recovered by a genuinely work-bounded, index-navigable scanner whose total query fan-out is centrally, non-configurably ceiling-bounded regardless of misconfiguration; no retry-eligible state class or processing-attempt bucket can ever be starved for any accepted parameter combination; `max_attempts = 0` disables retry/reclaim entirely with zero queries; and `retryable()` never silently returns more rows than its caller's own validated requested limit, rejecting an infeasible request outright instead.** §19, §26; `PaymentProviderEventRetryReclaimTest` in full (methods 13–14 for index-ordering, 15–23 for state-class fair-interleave/no-starvation, 24–28 for the per-attempt-bucket correction and its ceiling, 29–38 for the zero-disables-retry policy and the throw-based fairness validation); `PaymentProviderEventRetryPolicyTest` in full (cross-consumer normalization agreement); `ProviderRefundDisputeConcurrencyTest` in full; `RefundablePaidAvailableAccountingTest` method 13.
