...
4. `test_two_different_provider_event_ids_reporting_different_cumulative_refund_amounts_never_exceed_the_true_provider_cumulative` — races cumulative 60 against cumulative 100 for the same attempt/charge reference, using the locked causal-barrier mechanism above (never the plain pre-job start gate alone). Asserts: total `Refund` gross across both resulting rows equals exactly 100, never 160; total wallet debit is bounded by `min(100, refundable_paid_available_micro, available_balance_micro)`, never double-counted; `refundable_paid_available_micro` reflects exactly one 100-unit debit, not two independent debits; the sum of both rows' own `normalized_outcome_delta_micro`-equivalent gross deltas is exactly 100 (60 + 40, or 100 + 0, depending on commit order — never 60 + 100); no false `refund_exceeds_refundable_balance`/policy-excess suspension occurs when 100 is genuinely within the wallet's own available/refundable-paid balance; **both processed events' own `normalized_status` are asserted against the one, single, authoritative final persisted state (§17's own zero-delta-branch fix, above) — never a stale, per-process snapshot:** whichever event completes the full expected refund records `Refunded`; if the identical-total case (60 then 40, or a genuine no-op at 0) does not itself reach `expected_amount_micro`, its own event correctly records `Succeeded`, and a raced zero-delta loser (cumulative 60 arriving after cumulative 100 already committed the full refund) also records `Refunded`, matching the now-authoritative persisted state rather than the stale `Succeeded` the pre-fix defect would have recorded; if cumulative 60 genuinely applies first with no concurrent winner, its own contemporaneous `Succeeded` result remains valid and is asserted as such. The existing method 1 (identical-cumulative race, using the file's own unchanged pre-job start-gate mechanism — sufficient there, since both processes share one correlation key and the idempotency layer itself is what is under test, not the aggregate-read ordering) is retained, unmodified, proving the still-valid, separate idempotency-layer guarantee.
