...
- **`WorkspacePlanAssignmentRepository`** (contract + Eloquent): add `update(WorkspacePlanAssignment $assignment, array $attributes): WorkspacePlanAssignment` — M1's contract had `findByWorkspaceId`/`create()` only; `changePlan()`/`changePlanStatus()`/complimentary grant-revoke/`setAdditionalBusinessSlots()` all genuinely need to mutate an existing assignment row. Also add `hasNonComplimentaryForCatalog(int $catalogId): bool` — a lean existence check (`WorkspacePlanAssignment::where('workspace_plan_catalog_id', $catalogId)->where('is_complimentary', false)->exists()`) required by `updateCatalogPricing()`'s clearing guard (§13.L, this correction) to detect a non-complimentary reference while the catalog lock is held, without any entitlement-decision logic in the repository.
