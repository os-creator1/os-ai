...
- **Business-toggle create/remove (`disableBusinessFeature()`/`enableBusinessFeature()`) — corrected in this round, a genuinely distinct Business-scoped case, not merely "locks its Workspace like everything else":** the caller-supplied `Business` object's `workspace_id` cannot be trusted as current — a concurrent `reassignBusiness()` call may already have moved the persisted row. Exact order, matching `reassignBusiness()`'s own existing Workspace→Business lock direction exactly (never inverted): capture `expectedWorkspaceId` from the caller-supplied `Business` → lock that expected Workspace (`WorkspaceRepository::findForUpdate()`, else the existing `WorkspaceNotFoundException`) → lock/reload the Business by ID (`BusinessRepository::findForUpdate()`, the same existing method `reassignBusiness()` already uses — no new repository method) → if the Business no longer exists, throw the existing `WorkspaceBusinessNotFoundException` → if the freshly-locked Business's `workspace_id` no longer equals `expectedWorkspaceId`, throw the existing `BusinessWorkspaceMismatchException` (same constructor shape `reassignBusiness()` already uses — no new exception) → **corrected in this round: validate the locked Workspace's `is_active`, else the existing `InactiveWorkspaceMutationException` (RFC-003, reused verbatim — no new exception), mirroring RFC-003's own established "inactive Workspace cannot be customer-mutated" rule** → only then evaluate Workspace owner-or-active-Admin authority (§13.N) against the locked Workspace → every remaining decision/toggle operation uses the locked Business and Workspace, never the stale caller-supplied model (§13.K below).
...
10. Reassign + reassign racing the same destination Workspace's final slot (new in this round, RFC-004 §17.2 — the contract previously covered create+create and create+reassign but omitted reassign+reassign): two distinct source Workspaces, each holding a different Business, both race real cross-Workspace `reassignBusiness()` calls against the same destination Workspace, configured at its absolute effective-capacity boundary (`additional_business_slots = 2`, effective capacity `5 = business_slot_max`, exactly 4 existing Businesses — exactly one effective slot remaining) — executed concurrently via independent DB connections/processes, never sequential coincidence, using the existing ascending-Workspace-ID lock order (§6, unchanged, proven not altered). Exactly one reassignment succeeds and exactly one fails closed, deterministically with `BusinessSlotLimitExceededException` (the destination is already at its exact `business_slot_max` boundary once the winner commits — chosen deliberately for a deterministic max-boundary loser, not a production-semantics change); the destination's Business-row count increases by exactly one, never two; the winner's Business is authoritatively reassigned into the destination; the loser's Business remains authoritatively in its original source Workspace, completely unchanged; the loser performs no source membership-grant cleanup and no `reassignWorkspace()` write before its capacity check fails (§13.B's existing capacity-before-cleanup/write ordering is what makes this true, not a new guarantee); no unrelated Workspace's own count/decision is affected; no unhandled MySQL deadlock escapes.
11. Legacy onboarding + ordinary Business creation racing the same destination Workspace's final slot (new in this round, §13.C — specifically proves the newly-added explicit legacy Workspace lock genuinely serializes capacity-read-then-Business-insert against another real count-increasing operation, not merely against itself): fixture uses an **existing** Workspace — never the §13.D auto-provisioned-on-zero-candidates case — configured with a valid `active` entitlement at the exact effective-capacity boundary (`additional_business_slots = 2`, effective capacity `5 = business_slot_max`, exactly 4 existing Business rows — exactly one slot remaining); the legacy customer/resolver fixture state is set up so `resolveLegacyOnboardingWorkspace()` deterministically resolves to this exact existing Workspace as its single candidate (never the zero-candidate provisioning branch); a separate, independently-authorized `createBusinessInWorkspace()` call is prepared targeting the same Workspace. Races, concurrently via independent DB connections/processes, never sequential coincidence: (A) `BusinessManager::applyIdentity()`'s legacy CREATE path against this Workspace, and (B) `WorkspaceManager::createBusinessInWorkspace()` against the same Workspace. Both operations lock the same Workspace row via the same `WorkspaceRepository::findForUpdate()` serialization point (§13.C's new explicit lock and §13.A's existing lock respectively) — no inverse lock order exists between these two specific operations (unlike scenario 9's legacy-vs-`transferOwnership()` case), so no deadlock-retry policy applies here; this scenario proves ordinary lock serialization, not retry behavior. Because the fixture sits at the exact `business_slot_max` boundary, the loser deterministically receives `BusinessSlotLimitExceededException` once it observes the winner's committed count, exactly like scenario 10's own boundary technique. Proves: exactly one operation succeeds; exactly one fails closed after observing the winner's committed count; the final Workspace Business-row count is exactly 5, never 6; if legacy onboarding wins, its new Business authoritatively belongs to the destination and the ordinary `createBusinessInWorkspace()` call fails capacity; if ordinary creation wins, the legacy onboarding attempt creates no Business and fails capacity; the losing operation performs no Business insert of any kind; no duplicate Business is ever produced; no unrelated Workspace is affected; no unhandled deadlock escapes; the test remains bounded — no hang; no sleep/backoff of any kind is added to production code.
...
- Resolves the Core catalog row (same `tier = 'core'` lookup M1's own backfill used). Core must exist.
- **Corrected in this round (RFC-004 §10.1, mirroring `assignFirstPlan()`'s own existing inactive-destination rule exactly): before creating the assignment, the resolved Core catalog row's `is_active` must be `true`** — this is a **new** assignment (RFC-004 §10.1's "an inactive row cannot receive new assignments" applies exactly as it does to any other first assignment), so it is not exempt merely because it is complimentary/system-provenance. If `is_active` is `false`, the same existing global `\InvalidArgumentException` convention `assignFirstPlan()` already uses is thrown, with the identical message shape: `"Workspace plan catalog tier [core] is inactive and cannot receive new assignments."` — no new exception class. Because this is a complimentary/system assignment, no pricing validation is added by this correction — no catalog `price`/`currency_id` requirement, no `additional_business_slot_price_ratio` requirement — and no catalog row lock is acquired solely for this `is_active` check, matching `assignFirstPlan()`'s own existing complimentary-path precedent (no lock, unlocked initial read only) and M2's continued lack of any catalog `is_active` mutation anywhere. On rejection, no `workspace_plan_assignments` row, no `workspace_entitlement_transitions` row, and no `WorkspacePlanAssigned` event are ever created/dispatched — the method fails before any of them.
...
- Creates a `workspace_plan_assignments` row: `status = active`, `is_complimentary = true`, `additional_business_slots = 0` (a brand-new Workspace has no existing Businesses). **Corrected in this round — RFC-004 §10.3's own already-authoritative provenance requirement made explicit for this specific write:** `complimentary_reason` = the exact fixed reason string above (non-null, matching §10.3's "`complimentary_reason` required whenever `is_complimentary` becomes true"), `complimentary_granted_by_user_id = null` (§10.3's own narrow exception — this case exactly), `complimentary_granted_at` = the current timestamp — the **same** timestamp value used for this mutation's `created_at`/`updated_at` and the transition row's `created_at` below, one coherent `$now` captured once at the start of this method, not re-read per column.
- Writes one `plan_assigned` `workspace_entitlement_transitions` row (`actor_user_id = null`, `to_plan_catalog_id` = the Core row, `reason` = the same fixed string, `created_at` = the same coherent `$now`) — the normal first-assignment transition, no new transition type.
...
**Platform administrator only (§13.N).** **Corrected in this round — exact order, authority before any entitlement-state read, and inactive-catalog rejection added (RFC-004 §10.1):** Workspace locked first → `actorUserId` required and validated as a platform administrator (else `AuthorizationException`, before the Workspace's assignment state or any catalog row is ever read — a non-administrator never learns whether the Workspace is already assigned) → must currently be unassigned (else `WorkspacePlanAlreadyAssignedException`) → destination catalog resolved by tier → destination catalog must exist → destination catalog's `is_active` must be `true` on this initial read (else the existing `InvalidArgumentException` convention, message `"Workspace plan catalog tier [{$tier->value}] is inactive and cannot receive new assignments."` — matching the M1 repositories' own already-established `InvalidArgumentException` convention, §12; no new exception class) → for a **non-complimentary** assignment, the resolved catalog row is locked (`WorkspacePlanCatalogRepository::findForUpdate()`, after the Workspace lock — §6), `is_active` is **rechecked on the now-locked row** (authoritative for the write — a concurrent deactivation between the initial read and the lock must still be caught; same `InvalidArgumentException`/message if it flipped to `false`), and its `price`+`currency_id` must both be defined on the now-locked row (else `UndefinedPlanPricingException`, RFC-004 §12.5) — the assignment is created while the catalog lock is still held, so a concurrent `updateCatalogPricing()` clear cannot land between the pricing check and the write (§6 scenario 6); a **complimentary** assignment acquires no catalog lock and requires no pricing, **but is not exempt from the `is_active` requirement** — the initial-read `is_active` check above still applies and rejects a complimentary first assignment to an inactive tier → `additionalBusinessSlots` validated `{0,1,2}` for Core/Growth, must be `0` for Agency (else `InvalidAdditionalBusinessSlotsException`) → **corrected in this round (RFC-004 §12.5 — a first assignment's initial `additionalBusinessSlots` value is conceptually an increase from zero, so the same increase rule applies): for a `non-complimentary` Core/Growth assignment with `additionalBusinessSlots = 1` or `2`, the same already-locked destination catalog row's `additional_business_slot_price_ratio` must also be non-null (else `UndefinedPlanPricingException`, RFC-004 §12.5) — no second catalog lock is acquired, this reuses the lock already held for the base price/currency check above; `additionalBusinessSlots = 0` requires only `price`+`currency_id` as already stated — the ratio is never additionally required merely because the tier supports extra slots, only when a non-zero slot count is actually being represented. A `complimentary` assignment's `additionalBusinessSlots` of `0`/`1`/`2` remains fully exempt from this ratio requirement (and from `price`/`currency_id`), exactly like the base pricing exemption above. Agency's `additionalBusinessSlots` must remain `0` (enforced by the existing `InvalidAdditionalBusinessSlotsException` check above), and Agency's catalog row's `additional_business_slot_price_ratio` is legitimately always `null` (RFC-004 §8/§10.1) — this correction never blocks a non-complimentary Agency first assignment merely because that column is null, since Agency's own slot value is always `0` and this new rule only applies at `1`/`2`.** → `reason` required (non-empty) → assignment created → `plan_assigned` transition written → `WorkspacePlanAssigned` dispatched after commit.

**Corrected in this round — exact complimentary-provenance persistence (RFC-004 §10.3, made explicit for this specific write, no redesign):** when `isComplimentary: true`, the created row persists `is_complimentary = true`, `complimentary_reason = $reason` (the same already-required non-empty `reason` argument — no separate provenance-reason argument is introduced), `complimentary_granted_by_user_id = $actorUserId`, `complimentary_granted_at` = the current timestamp. When `isComplimentary: false` (a **paid** initial assignment), the created row persists `is_complimentary = false`, and **all three complimentary-provenance columns are explicitly `null`** — `complimentary_reason = null`, `complimentary_granted_by_user_id = null`, `complimentary_granted_at = null` — never fabricated, never copied from `$reason`/`$actorUserId` (those two arguments describe the *assignment* itself, not a complimentary grant, on the paid path).
...
`grantComplimentaryStatus()`: non-empty `reason` required. **Corrected in this round — exact provenance persistence and no-op field-preservation (RFC-004 §10.3, made explicit for this specific write, no redesign):**

- For a real `false → true` transition: sets `is_complimentary = true`, `complimentary_reason = $reason`, `complimentary_granted_by_user_id = $actorUserId`, `complimentary_granted_at` = the current timestamp; writes `complimentary_granted`; dispatches `WorkspaceComplimentaryStatusChanged` after commit.
- For an already-complimentary authorized no-op (already `is_complimentary = true`): no transition, no event — the no-op comparison itself only happens after the authority check, so a non-administrator never learns the current complimentary state — and, **corrected in this round: the existing `complimentary_reason`/`complimentary_granted_by_user_id`/`complimentary_granted_at` values are never overwritten or reset**. A plan change (§13.F) on an assignment that is already complimentary is likewise never treated as a new complimentary grant — it never touches these three provenance columns, preserving the original reason/actor/timestamp exactly as this section already requires. `revokeComplimentaryStatus()` itself is not redesigned by this correction (unchanged from the prior round, below).

**Never requires catalog pricing and never locks the catalog row** — granting complimentary status never creates a paid state, so RFC-004 §12.5's pricing invariant does not apply to it.

`revokeComplimentaryStatus()`: `reason` optional. **Corrected in this round (previously missing §12.5 consequence): revoking complimentary status turns the assignment into a non-complimentary one, exactly like an initial paid assignment or a plan change — after the authority check and the current-state read, and before flipping `is_complimentary` to `false`, the currently-referenced catalog row is locked (`findForUpdate()`, after the Workspace lock — §6) and its `price`+`currency_id` must both be defined on the now-locked row, else `UndefinedPlanPricingException` is thrown and the assignment remains complimentary and unchanged.** **Corrected in this round — the same already-locked catalog row's `additional_business_slot_price_ratio` requirement, when the assignment already carries extra slots (RFC-004 §12.5 — a complimentary assignment may legitimately have acquired `additional_business_slots` without pricing, but revoking complimentary status converts those slots into paid commercial capacity at that exact transition, so the ratio must exist then, not later):** if the current assignment's `additional_business_slots > 0`, the same already-locked catalog row's `additional_business_slot_price_ratio` must also be non-null, else the existing `UndefinedPlanPricingException` is thrown and the assignment remains complimentary and unchanged — no second catalog lock is acquired, this reuses the lock already held for the base price/currency check. When `additional_business_slots = 0`, only base `price`+`currency_id` are required, exactly as already stated — the ratio is never additionally required at zero slots. For Agency, `additional_business_slots` is always `0` by the existing invariant, so its legitimately-`null` ratio never blocks a paid Agency revoke when base `price`+`currency_id` are defined. On any validation failure (base pricing or ratio), the assignment stays complimentary, its provenance is completely unchanged, no transition is written, and no event is dispatched. When all required catalog fields are defined, revoke proceeds with the existing symmetric no-op/transition/event behavior (`complimentary_revoked`). Neither `grantComplimentaryStatus()` nor a successful `revokeComplimentaryStatus()` bypasses `inactive`/`suspended` status or slot capacity — both remain orthogonal gates exactly as RFC-004 §18 requires. No RFC-005 usage-cost waiver of any kind is invented beyond RFC-004's own existing complimentary semantics (§13's already-closed additional-slot-billing clarification).
...
3. If the Workspace no longer exists: throw the existing `WorkspaceNotFoundException`.
4. Lock/reload the Business by ID (the existing `BusinessRepository::findForUpdate()` — no new repository method).
5. If the Business no longer exists: throw the existing `WorkspaceBusinessNotFoundException`.
6. If the freshly-locked Business's authoritative `workspace_id` no longer equals `expectedWorkspaceId`: throw the existing `BusinessWorkspaceMismatchException`.
7. **Corrected in this round:** validate the **locked** Workspace is active — if `! $lockedWorkspace->is_active`, throw the existing `App\Exceptions\Workspace\InactiveWorkspaceMutationException($lockedWorkspace->id)` (RFC-003's own already-shipped exception, reused verbatim — no new exception class). This check reads `$lockedWorkspace`, never a caller-supplied or pre-lock value, and runs strictly after the Business/Workspace consistency check (steps 5-6) but strictly before authority (step 8) — a stale-Business mismatch still wins over an inactive-Workspace denial, since consistency is validated first.
8. Only after that active-state check passes, evaluate Workspace owner-or-active-Admin authority (§13.N) against the locked Workspace.
9. Use the locked Workspace and the freshly-locked Business for every remaining decision/toggle operation — never a stale caller-supplied model.
10. `disableBusinessFeature()` calls `decide()` using the locked Workspace, the locked Business, `$feature->value`, and `actorUserId`.
11. The toggle row is created/deleted against the freshly-locked Business.

This deliberately matches `reassignBusiness()`'s own existing Workspace→Business lock direction exactly (§6) — never a Business→Workspace inversion — and reuses its exact two existing consistency exceptions verbatim, plus RFC-003's existing `InactiveWorkspaceMutationException`; **no new exception class and no new repository method are introduced.** `EntitlementManager` never calls into `WorkspaceManager` for this check — it reads `$lockedWorkspace->is_active` directly, the same plain scalar column every other `is_active` check in this contract already reads. **This correction is scoped to the actor-driven Business-toggle mutation boundary only — `decide()` (§7.1) gains no `is_active` check of any kind and remains a pure entitlement-decision API, never a tenancy-access algorithm; RFC-003's own Workspace-active-state authorization remains that read path's independent, unmodified precondition.** A concurrent deactivation serializes correctly through the same Workspace row lock (step 2) without any new mechanism: if a `deactivateWorkspace()` call commits before the toggle acquires the lock, the toggle observes `is_active = false` and fails at step 7; if the toggle already holds the lock, the two operations simply serialize in the opposite order — no new concurrency test is required for this, since the existing Workspace-lock serialization already covers it.

Disable-only — absence means enabled subject to Workspace entitlement. Owner-or-active-Admin authority (RFC-003 §7.3, reused unmodified — `WorkspaceManager`'s existing `assertActorIsOwnerOrActiveAdmin()`-equivalent check, using `WorkspaceMembershipRepository::findByWorkspaceAndUser()` against the locked Workspace, step 8 above). Known feature required. The Workspace must currently be effectively entitled to the feature (via `decide()`, step 10 above) before a disable row can be created — there would be nothing meaningful to disable otherwise. Create/remove only — event-only (`BusinessFeatureToggleChanged`), **no** durable transition row, matching RFC-004 §21's own explicit "lower-stakes, more frequent than a Workspace-level change" justification for this one deliberate exception.
...
- **`EntitlementManagerFirstAssignmentTest.php`** — locked/unassigned-required/tier-resolved/pricing-invariant/slot-value bounds/reason/actor all required; duplicate first assignment rejected (`WorkspacePlanAlreadyAssignedException`); `plan_assigned` transition + `WorkspacePlanAssigned` event; **a non-platform-administrator actor is denied** (§13.N). **Corrected in this round — two explicit authority-precedence (state-conflict) proofs**: a non-administrator actor calling `assignFirstPlan()` against a Workspace that **already has an assignment** still receives `AuthorizationException`, never `WorkspacePlanAlreadyAssignedException`; a non-administrator actor calling it for a **non-complimentary** assignment against a catalog row with **undefined pricing** still receives `AuthorizationException`, never `UndefinedPlanPricingException` — proving the authority check runs before either piece of entitlement state is ever read. **Corrected in this round — inactive-destination-catalog proofs (RFC-004 §10.1)**: a **paid** first assignment to an inactive destination catalog tier is rejected (`InvalidArgumentException`); a **complimentary** first assignment to an inactive destination catalog tier is also rejected (proving complimentary is pricing-exempt but not `is_active`-exempt); an active destination still succeeds under the existing pricing rules, unaffected by this correction; the inactive-catalog denial occurs after platform-admin authority, so a non-administrator actor calling `assignFirstPlan()` against an inactive destination still receives `AuthorizationException` first, never `InvalidArgumentException`. **Corrected in this round — complimentary-provenance persistence proofs (§13.E, RFC-004 §10.3)**: an actor-driven complimentary first assignment stores the supplied `$reason` in `complimentary_reason`, stores the supplied `$actorUserId` in `complimentary_granted_by_user_id`, and stores a non-null `complimentary_granted_at`; a **paid** first assignment stores all three complimentary-provenance columns (`complimentary_reason`, `complimentary_granted_by_user_id`, `complimentary_granted_at`) as `null` — proven explicitly, not merely assumed absent. **Corrected in this round — non-complimentary initial `additionalBusinessSlots` ratio requirement (RFC-004 §12.5, §13.E)**: (A) a paid Core/Growth first assignment with `additionalBusinessSlots = 1` and `additional_business_slot_price_ratio = null` on the destination catalog is rejected with `UndefinedPlanPricingException`; (B) the same with `additionalBusinessSlots = 2` and a null ratio is likewise rejected; (C) a paid Core/Growth first assignment with `additionalBusinessSlots = 0`, defined `price`+`currency_id`, and a **null** ratio **succeeds** — proving the ratio is required because paid extra capacity is actually being allocated, not merely because the tier supports extras; (D) a paid Core/Growth first assignment with `additionalBusinessSlots` `1`/`2` and `price`+`currency_id`+ratio all defined succeeds; (E) a complimentary Core/Growth first assignment with `additionalBusinessSlots` `1`/`2` remains permitted with no `price`/`currency_id`/ratio defined at all, subject to the existing slot-value bounds; (F) a paid Agency first assignment with `additionalBusinessSlots = 0`, defined `price`+`currency_id`, and the catalog's legitimately-`null` `additional_business_slot_price_ratio` **succeeds**; (G) an Agency first-assignment request with a non-zero `additionalBusinessSlots` remains rejected by the existing `InvalidAdditionalBusinessSlotsException`, unaffected by this correction. The existing authority, inactive-catalog, provenance, transition/event, and duplicate-assignment tests in this file are unchanged by this correction.
- **`EntitlementManagerLegacyCompatibilityAssignmentTest.php`** — only reachable via `provisionWorkspaceRecord()`; exact fixed reason string; `additional_business_slots = 0`; null actor; `plan_assigned` transition (not `complimentary_granted`); event dispatched; never reachable for an ordinarily-created Workspace; **direct proof it cannot be invoked as a normal actor-driven self-grant by any Workspace owner, member, or platform administrator — no actor-supplied path reaches it** (§13.D/§13.N). **Corrected in this round — complimentary-provenance persistence proofs (§13.D, RFC-004 §10.3)**: the created assignment row's `complimentary_reason` equals the exact fixed legacy-compatibility reason string (not a generic/placeholder value); `complimentary_granted_by_user_id` is `null`; `complimentary_granted_at` is non-null; the `plan_assigned` transition's `reason` uses the same exact fixed string; and — reaffirmed, not newly introduced — no `complimentary_granted` transition is ever written for this initial-assignment case. **Corrected in this round — inactive-Core-catalog proofs (§13.D, RFC-004 §10.1)**: with an **active** Core catalog row, the existing compatibility-assignment proof succeeds exactly as already tested; with an **inactive** Core catalog row, the same call fails with `InvalidArgumentException`; on that failure, no `workspace_plan_assignments` row is created, no `workspace_entitlement_transitions` row is created, and no `WorkspacePlanAssigned` event is dispatched; the existing provenance assertions for the successful (active-catalog) case remain completely unchanged by this correction.
...
- **`EntitlementManagerComplimentaryStatusTest.php`** — reason required on grant; actor required both directions; correct transitions/events; does not bypass inactive/suspended or capacity; `grantComplimentaryStatus()` requires no catalog pricing and acquires no catalog lock under any condition; **corrected in this round: `revokeComplimentaryStatus()` against a catalog row with undefined `price`/`currency_id` is rejected (`UndefinedPlanPricingException`), and the assignment remains complimentary and unchanged — proven separately from a successful revoke against a catalog row with defined pricing, which proceeds with the existing transition/event behavior**; **a non-platform-administrator actor is denied for both grant and revoke** (§13.N). **Corrected in this round — authority-precedence (state-conflict) proof**: a non-administrator actor calling `revokeComplimentaryStatus()` against a catalog row with undefined pricing still receives `AuthorizationException`, never `UndefinedPlanPricingException`. **Corrected in this round — grant provenance and no-op field-preservation proofs (§13.H, RFC-004 §10.3)**: a real `false → true` grant stores the exact supplied `$reason` in `complimentary_reason`, stores the supplied `$actorUserId` in `complimentary_granted_by_user_id`, and stores a non-null `complimentary_granted_at`; an already-complimentary authorized no-op grant call **preserves the original `complimentary_reason`/`complimentary_granted_by_user_id`/`complimentary_granted_at` values completely unchanged** — proven by asserting all three remain byte-identical to their pre-call values, not merely that no transition/event fired; the existing change/revoke tests in this file remain intact and unmodified by this correction. **Corrected in this round — revoke's extra-slot ratio requirement (§13.H, RFC-004 §12.5)**: (A) a Core/Growth complimentary assignment with `additional_business_slots = 1`, base `price`+`currency_id` defined, and a **null** `additional_business_slot_price_ratio` is rejected on revoke with `UndefinedPlanPricingException`; (B) the same with `additional_business_slots = 2` and a null ratio is likewise rejected; (C) a Core/Growth complimentary assignment with `additional_business_slots = 0`, base `price`+`currency_id` defined, and a **null** ratio **succeeds** on revoke; (D) a Core/Growth complimentary assignment with slots `1`/`2` and `price`+`currency_id`+ratio all defined succeeds on revoke; (E) a failed revoke (either base pricing or ratio missing) is proven to leave `is_complimentary = true`, every complimentary-provenance column completely unchanged, no `complimentary_revoked` transition written, and no event dispatched; (F) an Agency complimentary assignment (`additional_business_slots = 0` by existing invariant) with base `price`+`currency_id` defined and the catalog's legitimately-`null` ratio **succeeds** on revoke. All existing authority, provenance, no-op, status, and base-pricing tests in this file are kept unchanged.
...
- **`EntitlementManagerBusinessToggleTest.php`** — must be currently entitled to create a disable row; disable-only (never a grant); create/remove only; event fires, no transition row is ever written; **full owner-or-active-Admin authority matrix**: ordinary Workspace Staff denied; an active Workspace Admin allowed; the Workspace owner allowed; an inactive Admin denied — and a platform administrator with no Workspace owner/Admin standing is also denied, since this mutation is never platform-administrator-gated (§13.K/§13.N). **Corrected in this round — stale-Business authoritative-revalidation proofs**: a stale `Business` object captured before a real `reassignBusiness()` move cannot `disableBusinessFeature()` using authority derived from the old Workspace — the call fails closed with `BusinessWorkspaceMismatchException` before any toggle row is created; the same stale object cannot `enableBusinessFeature()` (remove an existing toggle) using old-Workspace authority either, failing the same way before the toggle row is deleted; both proofs assert the failure occurs before any toggle mutation, not merely that the end result differs; a freshly-loaded `Business` in the new Workspace succeeds using only the new Workspace's owner-or-active-Admin authority, and an old-Workspace owner/Admin with no standing in the new Workspace is denied even against the freshly-loaded Business. **Corrected in this round — inactive-Workspace denial proofs (§13.K, RFC-003's established mutation rule)**: the Workspace owner attempting `disableBusinessFeature()` against an inactive locked Workspace receives `InactiveWorkspaceMutationException`; an active Workspace Admin attempting the same receives the identical exception; `enableBusinessFeature()` (removing an existing toggle) against an inactive Workspace likewise fails with `InactiveWorkspaceMutationException`; no toggle row is created or deleted on any inactive-Workspace failure; the inactive-state denial is proven to occur before owner/Admin authority is ever evaluated, matching RFC-003's established mutation ordering; a stale-Business mismatch is proven to still win over this active-state check — a Business that has moved to a different Workspace throws `BusinessWorkspaceMismatchException` rather than the caller's old (possibly inactive) Workspace's active state ever being inspected; behavior against a fresh, active Workspace remains completely unchanged by this correction. No new concurrency test is added for this — the existing Workspace-row lock (already exercised by scenario 8, §6) is what makes a concurrent deactivation serialize correctly, and this is asserted directly rather than re-proven under real concurrency.
