...
- `App\Jobs\Usage\ExpireStaleUsageReservations` (full M1 implementation) and `App\Jobs\Usage\EvaluateBusinessAutoRecharge` (M1-scoped minimal guard implementation — §5.7/§9 explain the exact, narrow reason this job must exist at M1 even though auto-recharge itself is M2+/M3+ scope).
...
- Any real auto-recharge charge attempt, any low-balance notification delivery, any receipt/invoice generation (`SendLowBalanceNotification`, `SendAutoRechargeDisabledNotification`, `SendReceiptNotification` are not dispatched by any M1 code path — §5.7).
...
- **Currency resolution — a genuine gap RFC-005 leaves unresolved at the RFC level (§39 item 10, "v1 settlement currency," explicitly non-blocking, recommends USD), which this M1 contract must now resolve deterministically, since M1 cannot create a wallet without a `currency_id`.** Direct evidence: `currencies` (`database/migrations/2018_12_01_191808_create_currencies_table.php`) has `code` (plain `string`, **no unique constraint**) and a nullable `user_id` FK (a legacy per-user custom-currency concept); `database/seeders/CurrenciesSeeder.php` seeds a `code = 'USD'` row (among eleven others) attributed to `user_id = 1`, only when explicitly run — **not** guaranteed present in a fresh `ultimatesms_testing` database, confirmed by `tests/TestCase.php` carrying no automatic `$this->seed()`/`DatabaseSeeder` call, and by RFC-004's own established test precedent (`Currency::query()->first() ?? Currency::create([...])`, found identically across `EntitlementManagerAdditionalSlotsTest.php`, `EntitlementManagerChangePlanTest.php`, `EntitlementManagerComplimentaryStatusTest.php`, `WorkspacePlanCatalogRepositoryTest.php`, and others) of creating its own `Currency` row directly rather than relying on the seeder. **Resolved for M1 (a recommended new design decision, category 3, since the RFC itself only recommends USD without pinning a resolution mechanism):** `businesses.currency_code` is **never** used to resolve a wallet's `currency_id` — RFC-005 §10 scopes the wallet's settlement currency platform-wide, independent of a given Business's own locale/display currency, which serves an unrelated purpose. `UsageWalletManager::initializeWalletForNewBusiness()` resolves `currency_id` via a new config key, `config('usage.default_currency_code')`, defaulting to `'USD'`, looked up as `Currency::query()->where('code', config('usage.default_currency_code'))->where('status', true)->oldest('id')->first()`; if no matching row exists, a new `UsageDefaultCurrencyNotConfiguredException` is thrown (a deployment/seed-data precondition failure, not a per-Business error) — never a silently invented currency row, never a per-Business guess from `currency_code`.
...
- **`EvaluateBusinessAutoRecharge` — a narrow, load-bearing exception, resolved deliberately, not by oversight.** RFC-005 §12's own ledger-insert rule is unconditional and structural: *"any ledger-entry insert with `available_delta_micro < 0` dispatches `EvaluateBusinessAutoRecharge` after commit."* Because a `Reservation` entry's own `available_delta_micro` is `-amt` (negative, by RFC-005 §13's own delta table), **the very first successful `reserve()` call at M1 already triggers this dispatch** — so the job class must exist at M1, even though real auto-recharge (a Stripe off-session charge) is M3 scope. **Resolved:** `App\Jobs\Usage\EvaluateBusinessAutoRecharge` ships at M1 with a **minimal, provably-inert M1-scope body**: its first and only executed statement is `if (! $wallet->auto_recharge_enabled) { return; }` — and `auto_recharge_enabled` can never be `true` for any wallet at M1, since no code path (no HTTP surface, no manager method) ever sets it, so the guard always returns immediately in any real M1-era environment. The job's full Stripe-charging body is explicitly out of scope, added only at M3. A direct test proves the M1 stub never calls any Stripe/payment class (§14).
...
| 10 | v1 settlement currency and multi-currency scope | **Indirectly yes — resolved narrowly for M1's own internal purposes only, not decided for the RFC as a whole.** M1 cannot create a wallet without *some* deterministic `currency_id`; §5.5 above resolves this via a new `config('usage.default_currency_code')` key defaulting to RFC-005's own recommended `'USD'`. This is an M1-scoped implementation resolution, not a final human decision on the open item itself — the open item remains open at the RFC level, and this contract makes no claim it is closing it permanently; a future human decision could still change the config value or introduce real multi-currency support without contradicting anything M1 built. |
...
**No item in this table is resolved by this contract as a final human product decision.** Item 10's M1-scoped config default is the one place this contract makes a concrete implementation choice in an area §39 leaves open — done narrowly, reversibly, and only to the extent M1's own schema requires a non-null value to exist.
...
| `currency_id` | `unsignedBigInteger`, FK `currencies.id`, `restrictOnDelete()` | No | — | resolved per §5.5, never from `businesses.currency_code` |
...
| `currency_id` | `unsignedBigInteger`, FK `currencies.id`, `restrictOnDelete()` | No | — | must match the wallet's v1 settlement currency (§5.5) |
...
**Algorithm, adapted from `WorkspaceEntitlementBackfillV1` (§5.4):**

- Query-builder-only (`DB::table('businesses')`, `'business_usage_wallets'`, `'currencies'`) — no Eloquent model, no model event.
- Resolves the platform default currency once, at the start of the run, via §5.5's exact algorithm; throws `UsageDefaultCurrencyNotConfiguredException` immediately (before touching any Business row) if unresolvable — a fail-fast precondition, not a per-Business failure.
...
- **Per-Business transaction** — each Business's own wallet insert commits or rolls back independently of every other Business.
- **Idempotent, safe under partial rerun** — a Business that already has a wallet is skipped (detected by the same left-join/not-exists query).
...
- **Final zero-unwalleted-Business assertion** — a `SELECT COUNT(*)` over Businesses still lacking a wallet must be zero at the end of a run; a non-zero count throws a new `UsageWalletBackfillIncompleteException` carrying the exact remaining count, mirroring `WorkspaceBackfillIncompleteException`/`WorkspaceEntitlementBackfillIncompleteException` exactly.
- **Per-Business wallet values**: `currency_id` → the resolved platform default (§5.5); `available_balance_micro`/`reserved_balance_micro`/`debt_balance_micro` → `0`; `spend_period_key`/`spend_period_start_utc`/`spend_period_end_utc` and `recharge_period_key`/`recharge_period_start_utc`/`recharge_period_end_utc` → constructed via §10.3's calendar-month algorithm, evaluated against **that Business's own `timezone` column** (confirmed non-nullable, §5.2), at the backfill run's own execution instant; `auto_recharge_enabled` → `false`; every other nullable column → `NULL`; `billing_status` → `active`.
...
**Supporting files** (mirroring `WorkspaceEntitlementBackfillResult`/`BackfillWorkspaceEntitlementsCommand` exactly): `App\Library\Usage\Migration\UsageWalletBackfillV1` (final class), `App\Library\Usage\Migration\UsageWalletBackfillResult` (final readonly class: `businessesProcessed: int`, `walletsCreated: int`, `remainingUnwalletedCount: int`, always `0` on success), `App\Console\Commands\BackfillUsageWalletsCommand` (thin wrapper, signature `usage:backfill-wallets`, no algorithm of its own).
...
**Seventy unique paths total.** Category subtotals sum exactly to the total; no path appears in more than one category: 9 migrations + 4 enums + 3 value objects + 7 models + 7 repository contracts + 7 Eloquent repositories + 1 gateway + 1 manager + 1 listener + 2 jobs + 5 exceptions + 1 config + 6 backfill/support (including the one modified `AppServiceProvider` entry) + 16 tests = **70**.
...
42. `app/Jobs/Usage/EvaluateBusinessAutoRecharge.php` (minimal M1-scope guard body, §5.7)

### Exceptions (5 new)

43. `app/Exceptions/Usage/UsageWalletNotFoundException.php`
44. `app/Exceptions/Usage/UsageReservationNotFoundException.php`
45. `app/Exceptions/Usage/InvalidReservationStateTransitionException.php`
46. `app/Exceptions/Usage/NoActiveRateForFeatureException.php`
47. `app/Exceptions/Usage/UsageDefaultCurrencyNotConfiguredException.php`

### Configuration (1 new)

48. `config/usage.php` — new file, exactly one key at M1: `default_currency_code` (default `'USD'`, §5.5).

### Backfill/support (6 new)

49. `app/Library/Usage/Migration/UsageWalletBackfillV1.php`
50. `app/Library/Usage/Migration/UsageWalletBackfillResult.php`
51. `app/Console/Commands/BackfillUsageWalletsCommand.php`
52. `app/Exceptions/Usage/UsageWalletBackfillIncompleteException.php`
53. `app/Exceptions/Usage/PlatformFeatureUsageClassificationBackfillIncompleteException.php` — mirrors §9.1's own final-assertion discipline; if migration 8's classification backfill is interrupted, this exception carries the exact remaining count, matching §9.2's wallet-backfill exception exactly.
...
55. `tests/Unit/Usage/UsageEnumsTest.php`
56. `tests/Feature/Usage/BusinessUsageWalletSchemaTest.php`
57. `tests/Feature/Usage/BusinessUsageRateSchemaTest.php`
58. `tests/Feature/Usage/PlatformFeatureUsageClassificationSchemaTest.php`
59. `tests/Feature/Usage/BusinessUsageReservationLedgerSchemaTest.php`
60. `tests/Feature/Usage/UsageWalletManagerReservationLifecycleTest.php`
61. `tests/Feature/Usage/UsageWalletManagerCommittedSpendFormulaTest.php`
62. `tests/Feature/Usage/UsageCalendarMonthRolloverTest.php`
63. `tests/Feature/Usage/UsageWalletManagerConcurrencyTest.php`
64. `tests/Feature/Usage/UsageWalletBackfillV1Test.php`
65. `tests/Feature/Usage/UsageWalletBackfillV1ConcurrencyTest.php`
66. `tests/Feature/Usage/BackfillUsageWalletsCommandTest.php`
67. `tests/Feature/Usage/NewBusinessWalletInitializationTest.php`
68. `tests/Feature/Usage/EvaluateBusinessAutoRechargeStubTest.php`
...
**Seventy total paths (items 1–70 above), matching §12's own opening count exactly.** **No unrelated path may be authorized.** If M1's own implementation discovers a genuine need for a path not listed above, that is a STOP-and-report condition for a bounded contract amendment (§17) — not something to add silently.
...
- **`UsageWalletBackfillV1Test.php`** (Feature): a wallet is created for every existing Business with the exact backfilled column values (§9.2); `UsageDefaultCurrencyNotConfiguredException` thrown fail-fast when no matching `currencies` row exists, before any Business is processed; final zero-unwalleted assertion passes; no legacy payment/billing table is touched.
...
- **`EvaluateBusinessAutoRechargeStubTest.php`** (Feature): dispatched after any ledger insert with negative `available_delta_micro` (directly verified on a successful `reserve()`); its M1-scope body returns immediately without any Stripe/`PaymentInstrumentManager`/HTTP call — a direct assertion no such class is referenced or invoked, the regression test for §5.7's own inertness claim.
...
5. Every existing Business, and every newly created Business via both confirmed creation paths, has exactly one wallet, proven by `UsageWalletBackfillV1Test`/`UsageWalletBackfillV1ConcurrencyTest`/`NewBusinessWalletInitializationTest`.
...
7. `EvaluateBusinessAutoRecharge` exists, is dispatched correctly, and is provably inert at M1, proven by `EvaluateBusinessAutoRechargeStubTest`.
8. No legacy payment/billing file, no controller/route/view, and no M2–M6 RFC-005 concept was touched, proven by mechanical searches 1, 7, 10, 11 (§13) and §16 exact-scope verification.
...
5. **Exceptions** (§12 items 43–47) and **config** (§12 item 48) — no dependency beyond PHP itself; may be built alongside step 4.
6. **`UsageWalletManager`, `RealUsageAuthorizationGateway`** (§12 items 38–39) — depends on repositories, exceptions, value objects.
7. **`AppServiceProvider` binding update** (§12 item 54) — depends on the gateway and all seven repository pairs existing.
8. **Events/listeners**: `InitializeBusinessUsageProfile` (§12 item 40) — depends on the manager.
9. **Initialization and backfill**: `UsageWalletBackfillV1`/`UsageWalletBackfillResult`/`BackfillUsageWalletsCommand`/its two exceptions (§12 items 49–53), migrations 8–9 (§12 items 8–9) — depends on the manager and the classification/wallet tables; migration 8 before migration 9 (classification rows are independent of wallets, but keeping the documented order avoids any ambiguity).
10. **Jobs**: `ExpireStaleUsageReservations`, `EvaluateBusinessAutoRecharge` (§12 items 41–42) — depends on the manager.
11. **Tests** (§12 items 55–70) — written alongside each corresponding step above, not deferred to the end; schema tests immediately after their migration, manager/lifecycle tests after the manager, backfill tests after the backfill action, the gateway/nine-key tests after the `AppServiceProvider` change.
...
- a human open decision is required (e.g., the exact `default_currency_code` value turns out unresolvable in a target deployment environment beyond what §5.5's fallback handles);
...
No such conflict was found while drafting this contract (§5's audit matched RFC-005 v1.4 exactly, for every item checked; §5.7's method-surface-boundary resolution is a narrow, documented, literal reading of RFC-005 §36's own M1 bullet, not a discovered conflict).
...
- all seven tables, their models, four enums, three value objects, seven repository contracts + Eloquent implementations, the manager, the gateway, the listener, the two jobs, the five exceptions, the config file, and the `AppServiceProvider` binding/repository additions are complete and match §6/§10/§12 exactly;
