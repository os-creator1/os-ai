...
## Second exceptional post-review factual/governance reconciliation

**This is NOT Correction Round 3.** `maximum_correction_rounds: 2` is unchanged. Ordinary correction rounds remain **2 of 2 consumed; 0 ordinary rounds remaining** — this second exceptional pass, like the first, neither resets nor increments that counter. **No product design change is made by this pass** — it corrects four factual/inventory/wording defects only.

Prior head: `d334a375331d611f803b8e529889eddc85c34f8e`.

**Reasons for this pass:**

1. **DTO owner-file counts were mislabeled as constructor-call-site counts.** Corrected below (§8) with both counts stated separately and exactly: `PaymentIntentResult` — 2 owner files, 6 direct constructor calls (all production); `CheckoutSessionResult` — 8 owner files, 12 direct constructor calls (4 production, 8 test) — re-derived via a corrected grep that also catches the unqualified fully-namespaced `App\Library\Usage\CheckoutSessionResult(` form used inside `ConcurrentTopUpConcurrencyTest.php`'s own heredoc-embedded child script, which the prior pattern silently missed.
2. **`ReceiptBoundaryTest.php`'s method-count arithmetic said 14 while its own named list contains 15** — mechanically recounted and corrected (§10/§14). No method was added, removed, or renamed to make the arithmetic fit; the pre-existing 15-method list is authoritative and unchanged.
3. **Stale wording referred to a "future M6 contract"** even though `docs/automation/RFC-005-M6-CONTRACT.md` is already human-merged (PR [#133](https://github.com/os-creator1/os-ai/pull/133), merge commit `31b16c55c9b2a3cc7fe1a8c34aa738ae348dddb4`, confirmed by direct `git log`/`git show` this pass). Corrected throughout (§0/§7/§11) to name the two genuinely not-yet-written M6 release-readiness artifacts that already-merged contract itself requires — `docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS-DEPLOYMENT.md` and `docs/automation/RFC-005-M6-CONFORMANCE.md` — rather than a nonexistent future contract. No M6 file is created, modified, or otherwise touched by this pass; M6's own already-merged contract and its Gap Rule are unaffected.
4. **The recipient-assertion tests still retained implementation discretion** ("`Notification::fake()` + `assertSentOnDemand(...)` or the repository's own established mail-assertion convention") despite a direct search finding no existing repository precedent to choose instead. Corrected (§10) to lock the exact, sole assertion strategy — `Notification::fake()` + `Notification::assertSentOnDemand(ReceiptAvailableNotification::class, fn (...) => ...)`, confirmed against the installed framework's own `NotificationFake` implementation — with no alternative offered.

Confirmed, re-stated explicitly: production allowlist remains **16** paths, test/support allowlist remains **11** paths, `ReceiptBoundaryTest.php` remains the only new test file, no PHPUnit test method was added, removed, or renamed by this pass, no implementation authorization exists, no M6 file is modified, M6 remains frozen, and `AI-AUTONOMY-STATE.json` is untouched. This governance branch still changes exactly one file.
...
- Locked:
  ...
  - `M6 remains frozen` — untouched, not resumed, no M6 document created or modified. **Note, corrected this pass: `docs/automation/RFC-005-M6-CONTRACT.md` already exists and is already human-merged** (PR [#133](https://github.com/os-creator1/os-ai/pull/133), merge commit `31b16c55c9b2a3cc7fe1a8c34aa738ae348dddb4`, confirmed by direct `git log`/`git show origin/main:docs/automation/RFC-005-M6-CONTRACT.md`) — "M6 remains frozen" means that already-merged contract, and the M6 release-readiness work it authorizes, is not resumed or touched by this branch; it does **not** mean a future M6 *contract* remains to be written. What genuinely does not yet exist are the two release-readiness *artifacts* that already-merged contract itself requires: `docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS-DEPLOYMENT.md` and `docs/automation/RFC-005-M6-CONFORMANCE.md`. **This correction places one exact requirement on those two future artifacts** (§11 item 5) — stated here, not written into any file by this branch.
...
5. **Therefore: the future RFC-005 M6 deployment guide (`docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS-DEPLOYMENT.md`) must state that production Usage-billing deployment requires a durable, non-`sync` queue connection for Receipt Boundary to be operationally recoverable, and the future M6 conformance matrix (`docs/automation/RFC-005-M6-CONFORMANCE.md`) must verify that requirement was actually carried forward into the deployment guide.** Corrected this pass: `RFC-005-M6-CONTRACT.md` itself already exists and is already human-merged (PR [#133](https://github.com/os-creator1/os-ai/pull/133), merge commit `31b16c55c9b2a3cc7fe1a8c34aa738ae348dddb4`) — there is no future "M6 contract" for this requirement to be carried into; it is these two already-named, not-yet-written M6 release-readiness *artifacts* that must carry it, once M6's own already-merged contract's release-readiness work is eventually authorized (still blocked pending all seven remediations, per M6's own Gap Rule — unaffected by this correction). This correction does **not** write that requirement into any file (M6 remains frozen, §0) and does **not** modify `config/queue.php`, `.env.example`, or `phpunit.xml`.
...
- **Corrected DTO-construction audit, mechanically re-verified this pass and re-stated with precise terminology (a second exceptional post-review correction — see the record below):** Round 2 conflated **owner-file counts** with **direct constructor-call counts**; both are now stated separately and exactly, re-derived via a corrected grep matching all three real call forms (bare `CheckoutSessionResult(`, `\App\Library\Usage\CheckoutSessionResult(`, and the unqualified fully-namespaced `App\Library\Usage\CheckoutSessionResult(` form used inside `ConcurrentTopUpConcurrencyTest.php`'s own heredoc-embedded child-process script, which the Round-2 grep pattern silently missed).
  - **`PaymentIntentResult`** — **2 owner files** (`app/Library/Usage/FakePaymentProviderGateway.php`, `app/Library/Usage/StripePaymentProviderGateway.php`), **6 direct constructor calls**, all production, zero in any test file: `StripePaymentProviderGateway.php`'s `createOffSessionPaymentIntent()`, `retrievePaymentIntent()`, `confirmPaymentIntent()` (one call each); `FakePaymentProviderGateway.php`'s `createOffSessionPaymentIntent()`, `retrievePaymentIntent()`, `confirmPaymentIntent()` (one call each).
  - **`CheckoutSessionResult`** — **8 owner files** (2 production, 6 test — unchanged list: `app/Library/Usage/FakePaymentProviderGateway.php`, `app/Library/Usage/StripePaymentProviderGateway.php`, `tests/Feature/Usage/TopUpStateMachineTest.php`, `tests/Feature/Usage/PayerChangeDuringPendingAttemptTest.php`, `tests/Feature/Usage/FundingAttemptPayerConsentTest.php`, `tests/Feature/Usage/FundingAttemptExactlyOnceWalletCreditTest.php`, `tests/Feature/Usage/ConcurrentTopUpConcurrencyTest.php`, `tests/Feature/Usage/AddonPurchaseTransitionAuditTest.php`), **12 direct constructor calls**: `FakePaymentProviderGateway.php` — 3 (`createCheckoutSession()`; `retrieveCheckoutSession()`'s unknown-session fallback; `retrieveCheckoutSession()`'s known-session/outcome branch); `StripePaymentProviderGateway.php` — 1 (its shared private `mapCheckoutSession()` helper, used by both `createCheckoutSession()` and `retrieveCheckoutSession()`); `TopUpStateMachineTest.php` — 1; `PayerChangeDuringPendingAttemptTest.php` — 1; `FundingAttemptPayerConsentTest.php` — 2; `FundingAttemptExactlyOnceWalletCreditTest.php` — 1; `ConcurrentTopUpConcurrencyTest.php` — 2 (one inside `confirmRunnerScript()`'s own embedded child script, one inside `test_duplicate_webhook_and_browser_return_credit_a_checkout_backed_top_up_exactly_once()`); `AddonPurchaseTransitionAuditTest.php` — 1. Production calls (4) + test calls (8) = 12.
  - Round 1's "exactly four construction sites" and Round 2's "8/2 direct construction sites" statements are both retracted as imprecise/false — this trailing-nullable-field addition remains fully backward-compatible at every one of these 18 combined call sites regardless, since none of them is broken by two new optional trailing parameters. This is an inventory-terminology correction only; it changes no production or test path in §13/§14.
...
- **`test_independent_billing_contact_receives_the_receipt_at_contact_email()`** — new this round (§R), exact assertion strategy locked this pass (no repository precedent for `Notification::fake()`/`assertSentOnDemand()` was found to choose an alternative — none exists, confirmed by direct search): a `business_billing_contacts` row with `contact_user_id: null`, explicit `contact_name`/`contact_email`. Uses `Notification::fake();` then, after the job runs:
  ```php
  Notification::assertSentOnDemand(
      \App\Notifications\Usage\ReceiptAvailableNotification::class,
      function (\App\Notifications\Usage\ReceiptAvailableNotification $notification, array $channels, object $notifiable) use ($expectedEmail) {
          return $notifiable->routes['mail'] === $expectedEmail;
      }
  );
  ```
  (confirmed against the installed framework's own `NotificationFake::assertSentOnDemand()`/`assertSentTo()`/`sent()` implementation: it asserts against `Illuminate\Notifications\AnonymousNotifiable`, whose `$routes` is a public array, and invokes the callback as `($notification, $channels, $notifiable)` — exactly this shape). `$expectedEmail` is the fixture's own `contact_email`.
- **`test_user_backed_billing_contact_receives_the_receipt_at_the_linked_user_email()`** — new this round (§R), same exact assertion strategy: `contact_user_id` set, `contact_name`/`contact_email` both `null` (matching `BillingProfileManager`'s own real write behavior). `$expectedEmail` is the linked `User`'s own `email`.

  Both tests may additionally assert `$notification->receiptUrl` (or equivalent) if `ReceiptAvailableNotification`'s own constructor exposes it as a readable property — a natural, no-cost strengthening of the same assertion, not a second testing strategy.
...
5. **New this round, stated as an operational requirement rather than a design gap:** production Usage-billing deployment must use a durable (non-`sync`) queue connection for Receipt Boundary to be operationally recoverable — to be carried by a human into the two not-yet-written M6 release-readiness artifacts (`docs/rfcs/RFC-005-BUSINESS-USAGE-BILLING-AND-WALLETS-DEPLOYMENT.md` and `docs/automation/RFC-005-M6-CONFORMANCE.md`) that RFC-005 M6's own already-merged contract (PR [#133](https://github.com/os-creator1/os-ai/pull/133)) requires, not written into any file by this branch (§7/§L).
...
11. `tests/Feature/Usage/ReceiptBoundaryTest.php` — new file, 15 methods.
...
**Preflight requirement, corrected this pass to state the main-process/child-process distinction precisely (it must not be read as implying `phpunit.xml` is inherited by spawned child processes, which it is not):**

- **Main PHPUnit process:** `phpunit.xml` supplies `QUEUE_CONNECTION=sync` directly; every successful funding-confirmation fixture reachable in this process must be receipt-safe (via the Fake's own deterministic default, or an explicit fixture correction), exactly as named in §10.
- **`ConcurrentTopUpConcurrencyTest.php`'s spawned Symfony `Process` children:** `phpunit.xml` supplies nothing to these — `baseRunnerPreamble()` itself must explicitly propagate `QUEUE_CONNECTION=sync` (via `putenv`/`$_ENV`/`$_SERVER`, never relying on any machine's own `.env.testing`), and the child's own runtime guard (`config('queue.default') !== 'sync'` ⇒ `exit(1)`) must verify it took effect, both exactly as locked in the exceptional post-review correction above.

Before running the full suite, the implementer must confirm every receipt-eligible funding fixture is safe under **its own actual execution environment** — the main process's `phpunit.xml`-supplied setting, or the child runner's own explicit, source-level setting — never assuming one implies the other. This contract's own §10 (and the exceptional correction above) is that confirmation for every path existing on the current base commit; the implementer's own job is to apply those corrections exactly as named, not to re-derive them.
